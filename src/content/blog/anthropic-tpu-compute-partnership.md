...
GPU/TPU の確保競争に注目が集まりがちだが、大規模 AI 推論・学習では **HBM（高帯域幅メモリ）の供給** がそのままアクセラレータの実効性能を左右する。Micron との複数年契約は、アクセラレータ（Google/Broadcom TPU、Amazon Trainium、SpaceX/Akamai の GPU 等）と並ぶ**メモリ/ストレージのサプライチェーン確保**であり、Anthropic のコンピュート戦略を「演算」だけでなく「記憶階層」まで垂直に押さえる動きと位置づけられる。Micron の Sumit Sadana CBO は「AI 革命はデータセンターからエッジに至るまでメモリ/ストレージの役割を恒久的に引き上げた」と述べた。なお供給契約の規模・期間や Micron の出資額はいずれも非開示。資本面（Series H 戦略投資・収益ランレート）の詳細は [Anthropic 大型資本調達ラウンド](/mdTechKnowledge/blog/anthropic-funding-2026/) を参照。
