...
category: "Claude技術解説"
tags: ["Claude Code", "バージョン履歴", "リリースノート", "アップデート", "Opus 5.5"]
excerpt: "Claude Code v2.0.59〜v2.1.282 のバージョン履歴。v2.1.282のmaxProseWidth設定・anthropic-skills/claude-ai名前空間スキルの読み込み停止（破壊的変更）、v2.1.281のMCP URL-mode elicitation・`/insights`のauto mode推奨表示、v2.1.280のClaude Opus 5.5追加・Pro/Team Standardの既定モデルOpus化・PermissionRequestフックの破壊的変更、v2.1.278のauto modeサーバーサイド分類器デフォルト化、v2.1.277のAGENTS.mdサポート、v2.1.275のCtrl+Enter即時送信・claude.aiスキル/プラグイン同期、v2.1.273のLLMゲートウェイ向けリクエストヘッダー・Remote Controlフォーク機能、v2.1.271のRemoteセッションfast mode対応・Bash/PowerShell/Monitorのper-command allowed_domains、v2.1.269の`claude plugin eval`・`/output-style`コマンド、v2.1.267の`maxEffortLevel`設定、v2.1.265のツール結果1GBキャップ、v2.1.263のバグ修正・安定性改善、v2.1.261の`/skill-doctor`・bashOutputMaxChars設定、v2.1.260の`/diff`差分パネル・`/advisor`テキスト形式、v2.1.259の`managedMcpServers`マネージド設定・並行セッション`~/.claude.json`巻き戻り修正、v2.1.258のmacOS 12起動失敗修正、v2.1.257のClaude Fable 5.1追加・既定モデル化、v2.1.252のバグ修正、v2.1.251の`PreModelSwitch`/`PostModelSwitch`フックイベント・Remote Controlへのツール呼び出しライブ配信、v2.1.248の`--restricted`フラグ・`experimental.cacheTtl`、v2.1.247の`SendFeedback`ツール・`/claude-api cost-optimize`、v2.1.246の`/permissions`Auto modeタブ追加、v2.1.245のLinux glibc 2.44クラッシュ修正、v2.1.243の`/usage`Loops内訳・`modelPicker`/`promptCacheTtl`/`modelPricing`設定、v2.1.239の`/claude-api upgrade`スキル（anthropic SDK 0.x→1.x自動移行）・Bedrockストリーミング二重課金修正・Alpine/muslアドオン対応、v2.1.238の`keybindingFlavor`設定（readline流Ctrl+W）・plugin marketplace `headersHelper`・self-hosted-runnerのgraceful shutdown/動的Proxy-Authorization・長時間セッションメモリリーク修正、v2.1.237の組み込み「Concise」出力スタイル・LLMゲートウェイ利用時のプロンプトキャッシュ修正、v2.1.236の`ANTHROPIC_DEFAULT_MODEL`環境変数・cross-session `notify_when_idle`通知・macOSサンドボックスワイルドカードread-deny改善、v2.1.235のオプションスペルチェック（aspell/hunspell/ispell）、v2.1.234の`CLAUDE_CODE_PROJECT_DIR_NAME`環境変数・`selection:clear`キーバインド・GitLab MRバッジのフッター/ステータスライン表示、v2.1.233のGitLab MR URL対応・`forward_user_identity`設定・Bashツールmemory cgroup（`CLAUDE_CODE_TOOL_MEMORY_LIMIT`）・`CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`、v2.1.232のサブエージェントforkデフォルト化・`@`による他セッションの名前メンション・GitLab対応強化（トークンマスキング/marketplace）・Fable 5の/advisor復帰・PowerShell/Git Bash権限バイパス修正、v2.1.229のremote-control --continue・plugin marketplace `command`ソース・SSE keepalive ping・workflowプレフィックススタガー、v2.1.228のclaude.ai同期スキルセキュリティ強化・Write上書きルール変更、v2.1.227のMaxプランFable誤課金促し修正、v2.1.224のSelf-Hosted Runner（自社インフラでのセッション実行）・200体サブエージェント上限撤廃・cross-session SendMessage・archiveプラグインソース、v2.1.225のcross-session SendMessage名前指定開始、v2.1.223のBashパーミッションバイパス・権限プロンプト隠蔽対策・workflowサンドボックス脱出の3件セキュリティ修正、`/review`の`/code-review`エイリアス化・Focus view（ツール実行折り畳みUI）・sandbox credential masking・worktree隔離のgitコマンド修正・Remote Control自動起動制限・ultraplan機能削除・Claude Opus 5 追加・デフォルト Opus モデル昇格・`sandbox.network.strictAllowlist`・`DirectoryAdded` hook・ネストサブエージェントフォワーディング・`workflowSizeGuideline` 設定キー・`/config key=value` でプロンプトから設定変更・auto モードでの破壊的 git コマンドのブロック・Bun 1.4 ランタイム・接続断時の部分レスポンス保持・WSL2/Windows Terminal スクロール修正・権限ルールのツールパラメータマッチング構文 `Agent(model:opus)`・ネスト `.claude/skills` の自動ロード・auto モードのサブエージェント起動前評価・会話言語でのセッションタイトル生成・`enforceAvailableModels` マネージド設定・`wheelScrollAccelerationEnabled`・サブエージェントが自身のサブエージェントを生成可能（最大5階層）・Claude Fable 5 へのアクセス追加・`--safe-mode` フラグ（カスタマイズ無効化でトラブルシュート）・`/cd` コマンド（プロンプトキャッシュを壊さない作業ディレクトリ変更）・`disableBundledSkills` 設定・fallbackModel 設定（最大3つのフォールバックモデル）・deny ルールの glob 対応・クロスセッションメッセージング堅牢化・既定思考モデルの thinking 無効化・requiredMinimumVersion/requiredMaximumVersion マネージド設定・/plugin list（--enabled/--disabled フィルタ）・バックグラウンドセッションの自動バージョン更新（コールドリスタート不要）・Dynamic Workflows トリガー語 workflow→ultracode 変更・シェル起動ファイル書き込み前プロンプト・acceptEdits モードでのビルドツール設定ファイル保護・/usage カテゴリ別内訳・allowAllClaudeAiMcps・/simplify→/code-review リネーム・claude agents --json・/resume バックグラウンドセッション対応・plugin パネル最終更新日・/model セッション単位化・plugin dependency enforcement・claude project purge・Agent View Research Preview・/goal コマンド・Plugin Marketplace・/tui・ANTHROPIC_BEDROCK_SERVICE_TIER・PR URL から /resume 検索・worktree.baseRef設定・.claude/skills plugin 自動ロード・Bedrock/Vertex/Foundry での auto mode opt-in など主要マイルストーンを解説。"
draft: false
---
...
| バージョン | 主な機能追加 |
|-----------|------------|
| **2.1.282（2026-09-24 PT）** | `maxProseWidth`設定追加（広いターミナルでのClaude地の文の幅上限、表・コードブロックは全幅維持）、テレメトリ変数の起動通知・`/status`/`claude doctor`表示追加（設定ファイル内で無視された/テレメトリを無効化したテレメトリ変数を一覧表示）、**破壊的変更**: `anthropic-skills`・`claude-ai`名前空間のスキルフォルダ・コマンドファイル・workflowコマンドが読み込まれなくなった（claude.ai同期スキルとの名前衝突対策。同名のプラグインは引き続き読み込まれる）、auto modeがtelemetry無効時の直結Anthropic API接続でもサーバーサイド分類器をデフォルト化、多数のバグ修正・改善（vim modeの複数バグ修正等） |
| **2.1.281（2026-09-23 PT）** | MCP URL-mode elicitation追加（2026-07-28プロトコル接続で、MCPサーバーがブラウザベースのフローをClaude Codeに開かせられるように）、`/insights`にauto mode推奨表示追加（直近セッションでauto modeが処理できたであろう権限プロンプト数を推定表示）、Claude apps gatewayに`assume_role`（Bedrock upstream向けSTS経由のIAMロール引き受け）・`guardrail`（Amazon Bedrock guardrailを全リクエストに適用）追加、`"attribution": false`設定追加（コミット・PR属性表示を一括非表示）、多数のバグ修正・改善（resume時のプロンプトキャッシュ破損問題群・MCP関連バグ多数等） |
| **2.1.280（2026-09-22 PT）** | **Claude Opus 5.5（`claude-opus-5-5`）追加・新デフォルトOpusモデルに**（1Mコンテキスト、$4/$20 per MTok、キャッシュ読込$0.20/MTok。詳細は[Claude Opus 5 完全ガイド](/mdTechKnowledge/blog/claude-opus-5-guide/)を参照）、**Pro・Team Standardプランの既定モデルをSonnetからOpusへ変更**（Max・Team Premium・Enterpriseと同じ扱いに統一）、`CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`追加（MCPツール説明・サーバー指示文の既定2,048文字上限を変更可能）、**破壊的変更**: `PermissionRequest`フックでagent-typeフックが動作しなくなった（判定結果を許可/拒否に反映できない性質のため。command/httpフックへの切替を促すエラー表示に変更）、多数のバグ修正・改善（v2.1.279はスキップ番号） |
| **2.1.278（2026-09-18 PT）** | auto modeがサーバーサイド分類器へ**デフォルト変更**（Claude API・Enterprise・Bedrock・Vertex・Foundry・ゲートウェイ対象。**分類器オーバーヘッドの課金なし**、Bedrock/Vertex/Foundry/ゲートウェイのみ`CLAUDE_CODE_AUTO_MODE_SERVER=0`でopt-out可）、`/status`に「Auto mode server」行追加（このセッションの分類器がサーバー側で動いているか表示） |
| **2.1.277（2026-09-18 PT）** | **AGENTS.mdサポート追加**（プロジェクトにCLAUDE.mdが無い場合Claude CodeがAGENTS.mdを読む。`/config`の「Project instructions」で切替可能、Bedrock/Vertex/Foundryは未対応）、Claude apps gatewayのforward proxy対応（`CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1`）、バックグラウンドタスク完了通知の待機表示追加、**サブエージェント結果を「サブエージェント出力」と明示するヘッダー付きで親エージェントに渡すように変更**（サブエージェント結果内のテキストがセッション自身の指示として通らないようにするプロンプトインジェクション対策）、非推奨のTaskOutputツール削除（Readで代替）、多数のバグ修正・改善 |
| **2.1.276（2026-09-17 PT）** | **v2.1.275のリグレッション修正**（`ANTHROPIC_BASE_URL`がプロキシ/ゲートウェイを指す場合、全リクエストが`400 Input tag 'advisor_20260301'`エラーで失敗する問題） |
| **2.1.275（2026-09-17 PT）** | Claude apps gatewayサインイン時にゲートウェイが名乗るアカウントを保存前に確認する仕組み追加（`/status`にも表示）、**Ctrl+Enter（またはCtrl+X Ctrl+S）の即時送信キー追加**（実行中ターンを中断しキュー中のメッセージを即座に一括送信）、**claude.aiアカウントのスキル/プラグイン同期機能追加**（ターミナルセッションにも反映、`syncClaudeAiSkills`/`syncClaudeAiPlugins`でopt-out可）、`/plugin install <plugin> --marketplace <source>`追加、多数のバグ修正・改善 |
| **2.1.274（2026-09-16 PT）** | メモリ使用量が重大な水準に達した際の警告表示追加、`CLAUDE_CODE_MCP_STARTUP_WAIT_MS`追加（起動時のMCPサーバー接続待機時間を調整）、OpenTelemetryスパン`claude_code.llm_request`に`effort`属性追加、Claude apps gatewayに`store.connect_timeout_seconds`追加（Postgres接続タイムアウト延長）、多数のバグ修正・改善（"unexpected tool_use_id"エラーでの無限リトライを自己修復・resume時の`/goal`消失問題等） |
| **2.1.273（2026-09-15 PT）** | **LLMゲートウェイ向け`x-claude-code-*`リクエストヘッダー追加**（`x-claude-code-request-class`等5種、`CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`でopt-in）、**MCPサーバーがセッション中に切断し自動再接続にも失敗した際の通知追加**（`/mcp`を案内）、**Remote ControlセッションをClaudeアプリからフォークする機能追加**（フォーク後はローカルPC上でバックグラウンドセッションとして実行継続）、多数のバグ修正・改善 |
| **2.1.272（2026-09-14 PT）** | バグ修正・安定性改善 |
| **2.1.271（2026-09-14 PT）** | **Remoteセッション（クラウド・self-hosted runner）へのfast mode対応**、**`/config`パネルのフルスクリーンモードにマウスサポート追加**（ホイールスクロール・クリックで値変更）、`claude self-hosted-runner --drain-marker-file <path>`追加、**Bash/PowerShell/Monitorへのper-command `allowed_domains`追加**（auto modeサンドボックス下でコマンドごとに許可ドメインを絞り込み）、**`omitClaudeMd`をagent frontmatter・`--agents` JSONに追加**（カスタム/プラグインサブエージェントがuser/project/local CLAUDE.mdを読まずに起動可能、managed policyファイルは引き続き読み込み）、多数のバグ修正・改善（org policyキャッシュのアカウント切替追従・動的workflow一時停止/自動再開等） |
| **2.1.270（2026-09-12 PT）** | Bashの読み取り専用gitコマンドが、セッション実行中しばらく経つと不意に権限を求め直すリグレッション修正（v2.1.269で混入） |
| **2.1.269（2026-09-11 PT）** | **`claude plugin eval`追加**（プラグインのevalスイートをClaude Codeに対して実行し、スコア付きで再現可能な結果をJSON+HTMLレポートで取得）、**`/output-style [name]`追加**（出力スタイルの一覧・切替。Remote Control・クラウド・headlessセッションでも利用可）、**`bashEditDiffEnabled`設定追加**（Bashツールがファイル編集を扱った際、変更ファイルの差分をBashツール結果に追加）、`OTEL_METRICS_INCLUDE_REPOSITORY`追加（OpenTelemetryメトリクス・イベントに`vcs.*`リポジトリ属性を付与）、`CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`追加（Workflowツールの実行あたり同時エージェント数上限を1〜256で調整）、多数のバグ修正・改善（プロンプトキャッシュ部分無効化・kitty-protocolターミナルのF1/F2/F4問題・リモート/headlessセッションのバックグラウンドエージェント待機表示問題等） |
| **2.1.268（2026-09-10 PT）** | Claude apps gatewayに`gateway.yaml`の`pricing:`設定追加（サインイン済みクライアントが同一料金をmanaged settings経由で受信、`/cost`とテレメトリが支出メーターと一致）、`gatewayInternalNetworks`マネージド設定追加、`claude self-hosted-runner --remove-session-state`追加、`claude plugin install/uninstall/update/enable/disable`に`--json`追加、**2.1.265以降のリグレッション修正**（サードパーティAnthropic互換エンドポイントで全ターンがHTTP 400になる問題等）、多数のバグ修正・改善 |
| **2.1.267（2026-09-09 PT）** | **`maxEffortLevel`設定追加**（トップレベルまたは`modelSettings`配下でモデル別に指定可能。Bedrock/Vertex/Foundryを含む全プロバイダでeffortレベルの上限をキャップ、ユーザーはそれより低いレベルを選択可能）、**`--system-prompt-snapshot off`追加**（会話に記録されたsystem promptを再利用せず、リクエストごとに新規レンダリング。プロンプト文面の反復調整向け）、多数のバグ修正（Coworkスケジュールタスクのサンドボックス起動失敗・MCP再接続/レジューム/モデル切替に伴うプロンプトキャッシュ無効化問題群・大容量セッションレジューム時の並行ツール呼び出し欠落等） |
| **2.1.266（2026-09-08 PT）** | v2.1.265で混入したLLMゲートウェイ・プロキシ環境のリグレッション修正（`CLAUDE_CODE_USE_GATEWAY`環境変数が単独指定時にCloud gatewayサインインを強制してしまい、APIキー等との併用構成で全リクエストが失敗する問題） |
| **2.1.265（2026-09-08 PT）** | **`user.email`/`user.groups`をテレメトリに追加**（Claude Desktop・CoworkがClaude apps gateway経由で送信するテレメトリに追加、端末セッションと同等に）、**`--plugin-dir`がプラグインの入ったフォルダを指定可能に**（マニフェストを持つ各子フォルダを読み込み、実行中の追加・削除も反映）、**ツール結果のディスク保存に1GBキャップ追加**（会話内プレビューは切り詰められた旨を表示）、多数のバグ修正（フォアグラウンド起動サブエージェントのレジューム時ツールリスト変更問題・エージェントチームメイトのプロンプトキャッシュ破損問題等） |
| **2.1.263（2026-09-05 PT）** | バグ修正・安定性改善（**2.1.262はスキップ番号、2.1.264もスキップ番号**） |
| **2.1.261（2026-09-04 PT）** | **`/status`・`claude doctor`に「Organization policy」行追加**（組織ポリシーが読み込めない理由〔プロキシがエンドポイントを通していない等〕を表示）、**`bashOutputMaxChars`/`taskOutputMaxChars`設定追加**（コマンド・バックグラウンドタスク出力をファイル保存前にインラインで受け取れる量を最大128K文字まで引き上げ）、**`--append-subagent-system-prompt-file`追加**（コマンドラインに載せきれない長いサブエージェントsystem promptをファイルから読込）、**`/skill-doctor`追加**（読み込み済みスキルのうち未使用のものとコンテキストコストを表示、不要スキルの整理に利用）、多数のバグ修正・改善（高速入力時の文字順序狂い・Remote Controlの権限モード表示ずれ・Bedrockセットアップウィザードのハング等） |
| **2.1.260（2026-09-03 PT）** | **`/diff`で会話横に差分パネル表示**（フルスクリーンモードで未コミット変更をClaude編集と同時にリアルタイム表示）、**`/cost`・ステータスライン`prompt_cache`フィールドにキャッシュミスの推定原因を追加**（ツール定義/system prompt変更・TTL経過後のアイドル等）、**`/advisor`のテキスト形式追加**（`/advisor`・`/advisor <model>`・`/advisor off`、デスクトップアプリ・Remote Control・headlessセッション〔`-p`/Agent SDK〕向け）、headlessセッションへの`/reload-plugins`追加、Fable 5.1関連のバグ修正多数（`/model`ピッカーにFable 5.1が表示されない問題・ツール結果後のコンテキストがプロンプトキャッシュされない問題・`/effort`変更でキャッシュが無効化される問題等）、`/ultrareview`の待機上限を30分→45分に延長 |
| **2.1.259（2026-09-02 PT）** | **`managedMcpServers`マネージド設定追加**（組織が全ユーザーにHTTP/SSE MCPサーバーを配布可能に。`.mcp.json`と同じエントリ形式、コマンド実行を指定するエントリはスキップ）、**無人headlessホスト向け`--permission-prompts none`追加**（プロンプトが発生するアクションを、有効な権限モード（auto modeを含む）の判定を維持したまま自動拒否）、`glab mr create/merge/close/reopen/note/update`の認識追加（GitLabマージリクエストが折り畳みツールサマリーで`MR !N`表示・フッターMRバッジ更新）、`claude plugin validate`に`--json`追加（機械可読な検証レポート）、**並行セッションが互いの`~/.claude.json`変更を静かに巻き戻す問題を修正**（多数セッション同時実行時にworkspace trustリセット・MCP/プロジェクト状態消失が発生しなくなった）、多数のバグ修正・改善 |
| **2.1.258（2026-09-01 PT）** | macOS 12（Monterey）でClaude Codeが起動しない問題を修正（v2.1.255で混入したリグレッション）、リモート/スケジュール済みセッションが再送された権限承認を適用できず「user messages must have non-empty content」で失敗する問題を修正 |
| **2.1.257（2026-09-01 PT）** | **Claude Fable 5.1（`claude-fable-5-1`）追加・Fableの既定モデルに**（1Mコンテキスト、$10/$50 per MTok、キャッシュ読み取り$0.25/MTok。詳細は[Claude Fable 5.1完全ガイド](/mdTechKnowledge/blog/claude-fable-5-1-mythos-5-1-guide/)を参照）、**「Time format」(`timeFormat`)/`timeZone`設定追加**（ターン終了時刻・トランスクリプトのタイムスタンプを12時間制/24時間制/24時間制UTC/strftimeパターンで指定可能）、**auto modeにContainment Escapeルール追加**（クラウドメタデータ認証情報の取得・egress回避・テナント越境アクセスを、環境が想定済みと明示しない限り自動承認しないように）、`CLAUDE_CODE_SUBAGENT_MODEL_FORCE`追加（`CLAUDE_CODE_SUBAGENT_MODEL`を個別起動・エージェント定義のモデル指定より優先して全サブエージェントに適用）、`/effort`に`s`追加（`/model`同様、現在セッションのみeffort変更）、auto modeで作業ディレクトリ外への初回ファイル読み取り前に確認プロンプト追加（`permissions.blockReadsOutsideWorkingDirectories`でブロック設定可）、多数のバグ修正・パフォーマンス改善（長い会話でのレンダリング性能改善、プロンプト入力のキー入力ごとの再描画削減 等）。VS Codeではセッションリストパネルのアカウント/使用量セクション折りたたみ、入力欄へのモデルピル追加、削除→アーカイブへの変更など |
| **2.1.252（2026-08-31 PT）** | Bashコマンドが一部のMacで「task output swap refused」で失敗する問題を修正、`.claude/settings.local.json`が未作成のプロジェクトで「常に許可」が保存されない問題を修正、Claude Desktop/VS CodeホストのRemote Controlセッションが接続劣化時に数分間ストールする問題を修正、非常に大きな失敗出力を含むバックグラウンドタスク通知がAPIリクエストサイズ上限を超える問題を修正 |
| **2.1.251（2026-08-28 PT）** | **`PreModelSwitch`/`PostModelSwitch` フックイベント追加**（モデル切替のブロック・確認・注釈付けが可能に。`SessionStart` resumeフックはセッションの陳腐化度と再キャッシュ推定コストも受け取るように）、**フォアグラウンド部分エージェントのツール呼び出し・結果をRemote Controlクライアントへライブ配信**（バックグラウンドサブエージェントは引き続きステータスのみ表示）、`/usage`に支出制限バー追加・ステータスライン`rate_limits.spend_limit`フィールド追加（gateway経由の支出上限利用者向け）、`/cost`にセッション単位のプロンプトキャッシュ行追加（ヒット率・ミス・再キャッシュトークン数・warm/cold）、`claude --help`に`attach`/`logs`/`stop`/`respawn`/`rm`追加、**ファイルツール（Read/Write/Edit）が権限チェック後に作業ディレクトリ内で差し替えられたシンボリックリンクを辿り、承認範囲外を読み書きできる問題を修正**、**プラグインマーケットプレイスエントリのコマンドがプラグインディレクトリ外を指せる問題を修正**（パストラバーサルエラーで拒否）、Workflowツール・Grep/Glob（シンボリックリンク経由）の権限チェック回避を修正、ほか多数のバグ修正・改善 |
| **2.1.250（2026-08-27 PT）** | バグ修正・安定性改善 |
| **2.1.248（2026-08-27 PT）** | **`--restricted`フラグ追加**（`CLAUDE_CODE_RESTRICTED=1`。コマンド実行ツール・WebFetchを除去、ファイルツールを作業ディレクトリ内に限定、`bypassPermissions`拒否、ユーザー/プロジェクト/ローカル設定ファイルを無視）、**エージェントfrontmatterに`experimental.cacheTtl`追加**（`"5m"`/`"1h"`でエージェント単位のプロンプトキャッシュTTLを設定）、`claude self-hosted-runner --client-label <label>`追加（ランナー登録ラベルのカスタマイズ）、長時間セッションで約1時間ごとに発生していたプロンプトキャッシュミス（OAuthトークン更新後のツール定義再レンダリングが原因）を修正、Claude Desktop/Coworkセッションが30日で消える問題を修正（`desktopSessionCleanupPeriodDays`設定でクリーンアップ猶予を調整可能に）、`/ultrareview`・シードされたcloudセッションが`prod.env`系・`*.tfvars`・エディタの一時/スワップ/バックアップファイル（`key.pem.tmp`等）の未コミット変更をアップロードしていた問題を修正、ほか多数のバグ修正・改善 |
| **2.1.247（2026-08-26 PT）** | **`SendFeedback` ツール追加**（セッション中に問題が起きた際、Claudeがフィードバック報告書を下書きし`/feedback`からレビュー・送信可能に。`feedbackDrafts`設定でオフ化可）、`spinnerTipsOverride`に`{id, text, cooldownSessions, priority}`エントリ・`tipsFile`・`label`を追加（組織が独自のヒントを組み込みヒントと併せてローテーション可能に）、Bash権限プロンプトにauto modeへのワンキー切替ヒントを追加、**`/claude-api cost-optimize` 追加**（既存プロジェクトのClaude API支出をプロファイルし、キャッシュ・トークン節約・バッチ・effort・モデル選択のコストレバーを1つずつ計測しながら検討）、`/claude-api`スキルをAdmin API対応に更新（組織メンバー・招待・ワークスペース・APIキー・レート制限レポート・workload identity federation・CMEK）、サブエージェントが初回モデル呼び出し404で死亡する問題を修正（セッションのフォールバックモデルチェーンを使用するよう変更）、フック/バックグラウンドエージェントの大量エラー出力が会話を溢れさせ「Prompt is too long」で行き詰まる問題を修正、ほか多数のバグ修正・改善 |
| **2.1.246（2026-08-25 PT）** | **`/permissions` に Auto mode タブ追加**（auto mode classifier rulesの閲覧・編集がGUIから可能に）、Bash allowルールにサブコマンド前ワイルドカード（例`Bash(git * main)`）の起動時警告追加、ターン完了時刻をターン所要時間表示の末尾に追加（例`✻ Sautéed for 23s · done 6:05 PM`）、長い1行diff（base64文字列等）を含む場合のトランスクリプト深刻な速度低下を修正、45秒後にバックグラウンドセッション起動が失敗する問題を修正（起動ディレクトリ削除・マシンスリープ・ホスト起動遅延時）、MCPツールコールがheadless/remoteセッションで割り込まれた際「完了（出力なし）」と誤報告される問題を修正、ほか35件超のバグ修正 |
| **2.1.245（2026-08-24 PT）** | Linux glibc 2.44 搭載ディストリビューション（Arch Linux・CachyOS・Fedora Rawhide等）での起動時クラッシュを修正 |
| **2.1.243（2026-08-24 PT）** | **`/usage` に Loops 内訳追加**（`/loop`タスクごとの実行回数・総トークン・実行あたりトークン・最終実行を表示、暴走/おしゃべりなループの特定が容易に）、**`modelPicker` 設定追加**（`/model`ピッカーに順序付き・ラベル付きモデル一覧をカスタム設定可能、Vertex/Bedrock ID含む）、**`promptCacheTtl`/`subagentPromptCacheTtl` 設定追加**（APIキー/クラウドプロバイダユーザーがメイン会話は1時間、サブエージェントは5分のプロンプトキャッシュを維持可能に）、**`modelPricing` マネージド設定追加**（組織の契約単価・割引率を`/cost`・ステータスライン・テレメトリのコスト計算に反映）、`/login`にAnthropic Consoleアカウントでのキーレスサインイン追加、`/status`に`Skipped sources`行追加（優先度の高いマネージド設定に隠れて未適用のソースを表示）、サブエージェントが実行したモデル・effortレベルを`/tasks`・エージェント詳細ダイアログに表示、ネイティブインストール/自動更新のダウンロードサイズ改善（Linux x64でzstd圧縮により約75MBに縮小、従来340MB）、多数のバグ修正・パフォーマンス改善 |
| **2.1.241（2026-08-23 PT）** | バグ修正・安定性改善 |
| **2.1.240（2026-08-22 PT）** | バグ修正・安定性改善 |
| **2.1.239（2026-08-21 PT）** | **`/claude-api upgrade` スキル追加**（Python `anthropic` SDK 0.x→1.x への自動移行を支援）、データレジデンシーワークスペースのコスト見積もりに1.1×プレミアムを反映、Bedrock/Vertex/Foundry 環境でもフルスクリーンレンダラーが利用可能に、Alpine/musl ビルドで画像ペースト・クリップボード・音声キャプチャのアドオンに対応、Bedrockストリーミング＋プロキシ環境でContent-Typeヘッダーが剥がされ二重課金が起きる問題を修正、JetBrains IDEターミナルでEdit/Writeが約5秒停止する問題を修正、ほか50件超のバグ修正・改善 |
| **2.1.238（2026-08-20 PT）** | **`keybindingFlavor` 設定追加**（`"readline"` 指定でプロンプト入力の `Ctrl+W` がBash流の「直前の空白まで削除」に。既定は `/` 等の記号で停止）、**プラグインマーケットプレイスの `headersHelper` 対応**（短命トークンをmintするローカルコマンドで認証ヘッダーを動的供給）、`claude self-hosted-runner` に `--defer-shutdown-max-min`（SIGTERM後も指定分数はアタッチ済みセッションを提供し続けるgraceful shutdown）と `--proxy-authorization-command` / `--proxy-authorization-file`（認証必須のegressプロキシへ動的に `Proxy-Authorization` ヘッダーを供給）を追加、長時間インタラクティブセッションでのメモリ増加（リーク）を修正、ほか35件超のバグ修正 |
| **2.1.237（2026-08-20 PT）** | **組み込みの「Concise」出力スタイル追加**（結論を先に示し前置き・ナレーションを省く。作業自体の徹底度は変えない。`/config`のOutput styleから選択）、LLMゲートウェイ／カスタムベースURLを使うセッションでのプロンプトキャッシュ不具合を修正 |
| **2.1.236（2026-08-19頃）** | **`ANTHROPIC_DEFAULT_MODEL` 環境変数追加**（新規セッションの開始モデルを指定。`/model`での選択は引き続き優先され再起動をまたいで永続化される点が`ANTHROPIC_MODEL`と異なる）、**cross-session `SendMessage`に`notify_when_idle`追加**（同一マシン上の別Claude Codeセッションへ「次にアイドルになったら1回通知して」と依頼可能。オプトイン・単発・ポーリングなし、macOS/Linux）、**サンドボックス改善**（macOSでワイルドカードread-denyルール〔例`**/.env`〕がallowed read領域内でも優先され、マッチしたディレクトリ配下の内容もカバー、リネームでの回避が不可に）、多数のバグ修正（セッションが切り替え先ディレクトリを削除された後クリップボードコピー/バックグラウンド処理/ローカルMCPログが壊れる問題〔2.1.229以来のリグレッション〕・フルスクリーンレンダラーが初回起動失敗後に永続的に失敗し続ける問題〔クラシックレンダラーへのフォールバックに修正〕・`/model`ピッカーがターミナル高を超えて表示される問題 等） |
| **2.1.235（2026-08-18 PT）** | **オプションのスペルチェック設定追加**（インストール済みの`aspell`/`hunspell`/`ispell`を使い、プロンプト入力中にスペルミスをリアルタイム下線表示）、修正多数（言語サーバーの切断/再接続時にプロンプトキャッシュ全体が無効化される問題・ネストされたmarkdownリストが深度3以降でインデントずれする問題・複数行プロンプトでのシンタックスハイライトが文字ずれする問題・権限プロンプトのコメント欄でShift+Tabが編集を承認しセッション全体の編集権限を付与してしまう問題・Agentツールが利用不可なセッションでも`subagent_type`省略時にgeneral-purposeを既定案内してしまう問題 等） |
| **2.1.234（2026-08-17 PT）** | **`CLAUDE_CODE_PROJECT_DIR_NAME` 環境変数追加**（プロジェクトごとに個別config directoryを持つホストで、per-projectトランスクリプトディレクトリに短い名前を指定可能）、**`selection:clear` キーバインドアクション追加**（アプリ内テキスト選択の解除をキーに割当可能、agents viewでも動作）、**GitLab MRバッジをフッター/ステータスラインに追加**（GitLabリモート＋認証済みglab CLIがあればMR `!N`をdraft/pending/green状態で表示）、claude.aiのusage limitリセット時にセッションを自動継続（`/config`の「Continue automatically at usage limit」でオフ可）、Windows NT-namespace（`\??\`）パスの拒否をリモートファイル読取・セッション復元・CLAUDE.md include・workflowスクリプト・ファイルアップロードにも拡大（NTLM資格情報漏洩ベクタ対策の強化）、多数のバグ修正（長時間セッションでのコンパクション後auto modeがサンドボックスネットワークアクセスを繰り返し拒否する問題・バックグラウンドサブエージェント権限プロンプト回答時のセッションスコープ権限が失われる問題・非ストリーミングfallbackパスでのクラッシュ・特殊Unicode列を含むメッセージのMarkdownレンダリング激重化・`SendMessage`が200文字上限/絵文字多用のセッション名を拒否する問題 等） |
| **2.1.233（2026-08-14 PT）** | **`--worktree`フラグと`claude agents`ビューにGitLab merge request URL対応追加**（MRは`!N`表示）、**`forward_user_identity` apps gatewayオプトイン設定**（Anthropicアップストリームでサインイン済みユーザーのidentityをヘッダー送信、ゲートウェイ背後のプロキシがユーザー単位で支出を帰属可能に）、**Bashツールコマンド向けmemory cgroupオプトインサポート追加**（Linux、`CLAUDE_CODE_TOOL_MEMORY_LIMIT`。暴走ビルドによるセッションストールを防止）、**`CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`環境変数追加**（WebFetchセッションURLキャッシュTTL設定、既定は15分のまま変更なし）、`claude self-hosted-runner`セッション起動時間の改善（作業ツリー書き換えなしでセッションブランチ作成・サーバーラウンドトリップ2回省略）、apps gatewayエラー転送の改善（Vertex/Foundry/Claude Platform on AWSの400/413エラーがアップストリーム本来のメッセージを保持）、修正多数（環境シャットダウン中の権限プロンプト待機でクラウドセッションがlost扱いになる問題・サーバーレスホスト等の固定タイムアウトでMCP v2接続が購読ストリームを無限再オープンする問題・Claude Desktop/VS Code下でNotification hookが権限プロンプトに発火しない問題 等） |
| **2.1.232（2026-08-14頃）** | **サブエージェントforkがデフォルト有効化**（`subagent_type: "fork"`が会話全体とプロンプトキャッシュを継承。インタラクティブセッションの非チームメイトagent起動は既定でバックグラウンド実行に）、**`@`で他のClaudeセッションを名前メンション**（SendMessageで直接連絡。完全一致する生存セッション名へはref確認なしで配信）、同一マシンのセッション名一意化（重複時は`name-word-word`変種を自動付与）、`/config`に「Dialog expiry」「Messages from your other sessions」（cross-session受信のaccept/hold/refuse）行を追加、**GitLab対応の大幅強化**（GitLabトークン群のシークレットマスキング・`glab` CLIに`gh`同等のサンドボックス/credential保護・plugin marketplaceのgitlab.com URL対応〔ネストサブグループ含む〕）、`additionalMarketplaces`/`allowedMarketplaces`エイリアス追加、**Fable 5が`/advisor`に復帰**（Fableアクセスのある組織向け・usage credits同意は`/model fable`経由）、**セキュリティ修正多数**（PowerShellの`$PSDefaultParameterValues`上書きによる権限バイパス・Git BashのCygwin形式シンボリックリンク経由の書き込みバイパス・ネストgitリポジトリの信頼継承・共有`/tmp`上のcross-sessionソケットディレクトリ強化・Linuxサンドボックスの保護パスバイパス）、Remote Control安定化多数（ネットワーク断後約30分の自動再接続・ブリッジセッションの履歴復元・別デバイスによる取得の明示・cloudセッション内ブリッジの資格情報継承修正）、`/code-review`のhigh/xhigh/maxもバックグラウンド実行に、`/plugin install`がmarketplaceを先に更新、フルスクリーンストリーミングの応答性改善 |
| **2.1.231（2026-08-13 PT）** | 事前登録済みOAuthクライアントを使うMCPサーバー（Slack等）でOAuthサインインがredirect URI不一致で失敗する問題を修正 |
| **2.1.230** | 公開CHANGELOGに個別エントリなし（欠番） |
| **2.1.229（2026-08-12 PT）** | **`claude remote-control --continue`**（最新のRemote Controlセッションを名前指定なしで再開）、self-hosted runnerセッションへのserver-supplied hook対応（managed環境と同挙動に）、**GatewayストリーミングにSSE keepalive ping追加**（Vertex/Bedrock上流での長い思考停止中のidle-timeout切断を防止）、**plugin marketplaceの`command`ソース追加**（IDE等のローカルコマンドがプラグインディレクトリを出力→セッション毎に再解決・再起動不要。`mode: "link"`でその場利用）、`ListAgents`が切断中のRemote Controlセッションを`offline`・クラウドセッションを`cloud`と表示、**workflowファンアウトの同一プレフィックス兄弟エージェントをスタガー起動**（後続がキャッシュ済みプロンプトプレフィックスを再利用。`CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS=0`で無効化）、`/commit-push-pr`で危険フラグ（`--force`/`--amend`/`--no-verify`等）付きgit/ghコマンドの自動承認を廃止、self-hosted runnerのWindows起動に`--base-dir`必須化、[VSCode]サイドバーにセッショングループ追加（右クリックで作成/改名/削除）・`/btw`パネルのリサイズ対応、auto modeが`CLAUDE_CODE_ATTRIBUTION_HEADER`無効化ユーザーで全ツール呼び出しに失敗する問題修正、`/model`がカスタムゲートウェイ利用のclaude.aiサブスクライバーでSonnet/Opus 1Mを誤拒否する問題修正、MCP OAuthのstrict認可サーバー対応（redirect URIに`127.0.0.1`使用）、32MB超過会話の圧縮リトライ暴走修正など修正多数 |
| **2.1.228（2026-08-11頃 PT）** | **claude.ai同期スキルのセキュリティ強化**（ローカルコマンド・MCPプロンプトのシャドウイング防止、説明文のサニタイズとラベル付け、本文の`!`コマンド実行・`@`ファイル展開の無効化）、**Writeツールの挙動変更**（新しいモデルは今セッションで未読の既存ファイルも上書き可能に。Editツールと同ルール化。旧モデルは従来どおり事前Read必須）、cross-sessionメッセージのインライン表示改善（送信者と本文を折り畳まず表示）、Vertex AI資格情報の期限切れ時に数秒で失敗するよう改善、auto modeの「わずかに高コスト」注記を初回通知から削除、描画停止・WindowsのGit Bash検出・`/tui`のモデル巻き戻り・セッションクリーンアップによるメモリフォルダ削除・marketplace設定マージなど修正多数 |
| **2.1.227（2026-08-09頃 PT）** | 期限切れログイントークンでセッション開始した際にfeature flagがサブスクリプションティアを評価せず、**MaxプランユーザーにFable用usage credits有効化を誤って促すバグを修正**、GitHub-hosted runnersで`allowed_non_write_users`使用時に全Bashコマンドが失敗する問題（`claude-code-action`）を修正、`/tui`が最初のメッセージ以前に巻き戻した会話を復活させる問題修正、スラッシュコマンドメニューの視認性改善（選択行のみ青・一致文字は太字）、パフォーマンス改善 |
| **2.1.226** | バグ修正・信頼性改善（特筆すべき機能追加なし） |
| **2.1.225（2026-08-08頃）** | ゲートウェイ支出上限のusage warning対応（上限額・リセット時刻・運用者メッセージを表示）、未信頼ディレクトリでの`claude agents`にワークスペース信頼プロンプト追加（`claude`本体と同挙動に統一）、長時間OAuthトークン切れ後のheadlessセッション破損を修正、macOSのMCP OAuth間欠401エラー修正、**cross-session `SendMessage`が名前指定で会話を開始可能に**（`ListAgents`で`name [ref]`表示、従来は相手からのメッセージへの返信のみ対応）、`claude self-hosted-runner`の`--base-dir`作成失敗時に起動時エラー表示するよう修正、[VSCode] Focus viewが最新のto-doリスト・保留中の質問を折り畳んでしまう問題を修正 |
| **2.1.224（2026-08-07 PT）** | **Self-Hosted Runner追加**（`claude self-hosted-runner`コマンドで自社マシン・コンテナをClaude Code Web/モバイル/デスクトップセッションの実行先に変換。Team/Enterprise限定）、**`archive`プラグインソース追加**（git/npm不要でzip over HTTPSからプラグインをインストール、SHA-256ピン留め対応）、**サンドボックスcredential maskingオプション拡充**（構造化env値向け`extract`/`onExtractNoMatch`、JWT対応`decode: "jwt"`＋`maskClaims`、AWS SigV4再署名向け`awsPairs`/`sigv4`。`network.tlsTerminate`必須）、**cross-session `SendMessage`追加**（Claude Codeセッション同士がマシンを跨いでメッセージ送信可能に、`ListAgents`で発見。macOS/Linux対応）、`crossSessionInbound`/`dialogExpiry`設定追加（bypass権限セッションへのクロスセッションメッセージは承認待ちに）、**200体サブエージェント/セッション上限を撤廃**（同時実行数・深度の上限は引き続き適用）、`ANTHROPIC_BEDROCK_REGION_PREFIX`環境変数追加（Bedrockのクロスリージョン推論プロファイル指定）、長い（200文字超）プロジェクトパスがセッションディレクトリを混同する問題等、多数のセキュリティ・信頼性修正 |
...
| **2.1.219（2026-07-24 PT）** | **Claude Opus 5（`claude-opus-5`）追加・デフォルト Opus モデルに昇格**（詳細は [Claude Opus 4.8 完全ガイド](/mdTechKnowledge/blog/claude-opus-4-8-guide/) の後継案内、[Anthropic Messages API 新機能まとめ](/mdTechKnowledge/blog/anthropic-messages-api-new-features-2026/) を参照）、**`sandbox.network.strictAllowlist` 設定追加**（許可リスト外のホストへの通信をプロンプトなしで拒否）、**`DirectoryAdded` hook 追加**、**サブエージェントのネスト生成デフォルトが深度3まで許可に緩和**（v2.1.218でデフォルト無効化されていたのが復活、`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1`で再無効化可。詳細は[Claude Codeサブエージェントの歩き方](/mdTechKnowledge/blog/claude-code-subagent-readonly-switch/)）、stream-json出力でのネストサブエージェントフォワーディング追加（`--forward-subagent-text`指定時）、**Opus 4.7 の fast mode 削除**により `/fast` は Opus 5 / Opus 4.8 のみ対応に（詳細は [Claude Opus 4.7 完全ガイド](/mdTechKnowledge/blog/claude-opus-4-7-guide/)）、**`workflowSizeGuideline` 設定キー追加**（Dynamic Workflows の既定サイズガイドラインを設定ファイルから調整可能に。詳細は [Claude Code Dynamic Workflows 完全ガイド](/mdTechKnowledge/blog/claude-code-dynamic-workflows-guide/)） |
| **2.1.218** | **`/code-review` がバックグラウンドサブエージェントとして実行**されるように変更、**サブエージェントのネスト生成をデフォルト無効化**（`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`で明示的に許可する必要に。v2.1.219で深度3まで再許可、詳細は[Claude Codeサブエージェントの歩き方](/mdTechKnowledge/blog/claude-code-subagent-readonly-switch/)）、**同時実行サブエージェント数の上限追加**（既定20、`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`で調整可）、`--max-budget-usd`到達時にバックグラウンドサブエージェントが停止しない問題を修正、**`/deep-research`が手動起動のみに変更**（Claudeが自律的に起動しなくなった）、**Plan Modeのautoが静的解析で読み取り専用と証明できないBashコマンドを無確認実行しなくなり、auto-mode分類器が判定するよう変更**、スクリーンリーダーモード改善（単語/行削除時のアナウンス追加）、Windows パスの `\u` プレフィックスセグメント文字化け修正、左矢印キーでの会話破棄に確認プロンプト追加（詳細は[Claude Code Tips①](/mdTechKnowledge/blog/claude-code-tips-1-left-arrow-agent-view/)）、Bedrock モデルの価格計算修正、MCP認証・接続系の複数修正など多数のバグ修正リリース |
...
| **2.1.206** | **`/cd` にディレクトリ候補補完を追加**（`/add-dir` と同挙動）、**`/doctor` に「チェックイン済み CLAUDE.md のトリミング提案」チェックを追加**（Claude が導出できる内容の削減）、`/commit-push-pr` が設定済み push リモートへの `git push` を自動許可、**`EnterWorktree` がプロジェクトの `.claude/worktrees/` 外の worktree 侵入時に確認プロンプトで保護**、**背景エージェントのバージョンアップが Claude Code 更新直後にバックグラウンドで完了**、期限切れログインが全モデルで誤解を招くエラーになる問題を修正 |
| **2.1.205** | **auto mode にセッション・トランスクリプトファイルの改ざんをブロックするルールを追加**、`--json-schema` が不正スキーマ時に非構造化出力を黙って返す問題を修正、`--max-turns` 到達時に作業中に送られたメッセージが失われる問題を修正、**Windows の worktree 削除が NTFS ジャンクション/シンボリックリンク経由で worktree 外のファイルを消す問題を修正**、背景エージェント/agent list まわりの修正 |
| **2.1.204** | バグ修正・信頼性改善（特筆すべき機能追加なし） |
| **2.1.203** | **ログイン期限切れの警告を追加**（背景セッションが中断される前に再認証できる）、**Manual 権限モード時にフッターへグレーの ⏸ バッジを表示**（現在モードを常時可視化）、MCP `roots/list` にセッションの追加作業ディレクトリを含め変更時に `notifications/roots/list_changed` を送信、多数の背景セッション/デーモン/worktree 分離の安定性修正（macOS の誤った低メモリ検知による15〜20秒スタール、デーモン token 失効で応答不能、worktree 分離サブエージェントが親チェックアウトで実行、`claude agents` へ戻るとサブエージェントが停止し最初からやり直す問題 等）、サブエージェントの再委任抑制、起動メモリ・バイナリを各約7MB削減、`/clear` 後に Windows の背景タスク出力が空になる問題を修正 |
| **2.1.202** | **`/config` に「Dynamic workflow size」設定を追加**（small/medium/large でワークフローのエージェント数規模を制御）、**ワークフロー生成エージェントの OpenTelemetry に `workflow.run_id` / `workflow.name` 属性を追加**（ワークフロー実行の追跡が容易に）、`/workflows` のエージェント一覧レイアウト改善（タイトル幅拡大・時間列・短いモデル名）、Ctrl+R 履歴検索のスキャン中確定/取消でのクラッシュ修正、mTLS ハンドシェイクの一時失敗修正、背景セッション rename の復帰バグ修正、re-invoke したスキルが指示を重複追加する問題を修正、`/review <pr>` を高速な単一パスレビューに戻す |
...
| **2.1.178** | **権限ルールにツールパラメータマッチング構文 `Tool(param:value)` を追加**（ツールの入力パラメータに `*` ワイルドカードでマッチ。例: `Agent(model:opus)` で Opus サブエージェントの起動をブロック）、**ネストした `.claude/skills` ディレクトリのスキルが、その配下のファイルで作業する際に自動ロード**（名前衝突時は `<dir>:<name>` 形式で表示され双方が利用可能）、**ネスト `.claude/` の衝突解決**（作業ディレクトリに最も近い agent / workflow / output-style が優先。プロジェクトスコープの workflow 保存は最も近い既存 `.claude/workflows/` を対象）、**auto モード改善: サブエージェントの起動を起動前に分類器が評価**（サブエージェントがレビューを経ずにブロック対象アクションを要求できる抜け穴を解消）、`/doctor` のレイアウト改善（全セクションで一貫したフラットツリー表示・ステータスアイコン明確化・コマンド名のハイライト）、**Dynamic Workflows のトリガー語を紫シマーでハイライトし、"run a workflow" / "workflow:" など明示フレーズでのみ起動**（語の単なる言及では起動しない）、`/bug` が送信前に説明を必須化（モデル拒否テキストを GitHub Issue タイトルに使わない）、**Agent Teams の大改訂: `TeamCreate` / `TeamDelete` ツールを廃止し、`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 有効時は各セッションが暗黙の1チームを持つモデルへ変更（セットアップ不要で `Agent` ツールの `name` パラメータから直接チームメイトをスポーン）**、多数の修正（stale な websocket/OAuth ファイルディスクリプタ継承による OOM クラッシュ、Claude in Chrome の OAuth アカウント不一致での無音接続失敗、非対話実行でディレクトリ修飾名のネストスキルが権限プロンプトでブロックされる問題、サブエージェント関連〔トランスクリプトにツール結果／ライブ進捗を表示・完了処理中に送ったメッセージの欠落・ctrl+b の背景化で最初からやり直しになる問題〕、カスタム API ゲートウェイ起動時に `claude agents` ワーカーが 401 になる問題、`--fallback-model` を尊重しないコンパクション、認証情報更新後も stale キャッシュで失敗し続ける問題、`disallowedTools` の MCP サーバーレベル指定〔`mcp__server` 等〕が無視される問題、vim モードの undo 等）。**※ v2.1.177 はスキップ番号**（公式 changelog は 2.1.176 → 2.1.178） |
...
> **※ 詳細記載は 2.1.176 まで**。2.1.177 以降の各バージョン（〜最新 2.1.207）は上記「主要マイルストーン一覧」を参照してください。

### 2.1.176（2026-06-12 PT）— **会話言語でのセッションタイトル生成 / `availableModels` 強制の堅牢化**
