...
excerpt: "Claude Managed Agentsの3層アーキテクチャ（Session/Harness/Sandbox）、p50 TTFT 60%削減のパフォーマンス改善、2026年4月下旬パブリックベータ移行のMemory機能、エンタープライズ向けRBAC・OpenTelemetry対応、料金体系（$0.08/session-hour）までを1ページに整理。"
draft: false
---
...
2026年4月下旬には**Memory機能がパブリックベータへ移行**し、エンタープライズ向けにはRBAC・グループ支出制限・利用分析・OpenTelemetry連携などの管理機能が追加されました。
...
## Memory機能（2026年4月下旬パブリックベータ）

2026年4月下旬、**Memoryがパブリックベータへ移行**しました。Managed Agentsにおける長期記憶レイヤーとして、以下の特徴を持ちます。

| 観点 | 内容 |
|------|------|
| **実装方式** | エージェントの**ファイルシステム層**として実装。エージェント自身が読み書きできるファイル群として扱える |
| **学習・改善** | セッション間で経験を蓄積し、繰り返し利用するほど精度・効率が向上 |
| **監査性** | Memoryへの**全変更がAudit logに記録**される。コンプライアンス要件にも対応 |
| **管理性** | API経由で**エクスポート・インポート・削除**が可能。組織ポリシーに沿った運用が可能 |
...
- **Research Preview 機能**（マルチエージェント連携 / 自己評価 outcomes 等）は引き続き別途アクセス申請が必要 → [claude.com/form/claude-managed-agents](https://claude.com/form/claude-managed-agents)
- **Memory** はパブリックベータへ移行済みのため、申請不要で利用可能
...
5. Research Preview機能（マルチエージェント連携・自己評価など）が必要な場合は [claude.com/form/claude-managed-agents](https://claude.com/form/claude-managed-agents) から申請
6. Memoryはパブリックベータのため申請不要。エンタープライズ向けRBAC/支出制限/OpenTelemetryはConsoleの管理画面から設定

---

*Sources: Anthropic Blog, Claude API Docs（platform.claude.com/docs/en/managed-agents/overview）, SDTimes, TestingCatalog, 9to5Mac, WaveSpeed — April 2026*
*本ガイドは2026年4月26日時点の情報に基づいています（2026-04-08 GA / 2026-04-09 Enterprise / 2026-04下旬 Memory PB を反映）*
