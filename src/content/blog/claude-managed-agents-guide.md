...
| Session-level overrides | **アクティブなセッション**の MCP サーバー・ツール構成をその場で更新可能 |
| Vault 環境変数クレデンシャル | シークレットを**環境変数としてサンドボックスに注入**（環境変数認証の CLI/SDK 向け） |
| Webhook 拡張 | agent／deployment／deployment run の各ライフサイクルをカバー（新バージョン公開・停止・スケジュール失敗をポーリング不要で検知） |
