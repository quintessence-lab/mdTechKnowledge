...
excerpt: "2026年6月15日、Anthropic は Agent SDK・claude -p（headless）・Claude Code GitHub Actions・サブスク認証のサードパーティ製アプリを、サブスクリプション共有枠から独立した『Agent SDK クレジットプール』（API標準レート課金）へ分離した（施行済み）。対象/対象外の切り分け、プラン別クレジット（Pro $20／Max 5x $100／Max 20x $200 ほか）、ロールオーバーなし、超過時の挙動、一回限りのオプトイン（claim）必須、そして移行に向けた棚卸し手順までを公式ヘルプセンター記事（一次ソース）ベースで整理する。【2026-06-16更新】施行日が到来し、対象の自動実行は既に新クレジットプール課金体制下にある。"
draft: false
---

> ## ⚠️ 要点（先に結論）
>
> - **2026年6月15日**、Agent SDK・`claude -p`（headless）・Claude Code GitHub Actions・サブスク認証のサードパーティ製アプリが、**サブスクリプションの共有枠から切り離され、独立した「Agent SDK クレジットプール」（API標準レート課金）へ移行**しました（**施行済み**）。本記事更新時点（2026-06-16）で、対象の自動実行は既に新体制下で課金されています。
...
2026年6月15日は、Claude を「自動で動かしている」開発者にとって**課金体系の節目**でした（**本記事更新時点〔2026-06-16〕で施行済み**）。これまで Pro / Max などのサブスクリプションでは、対話的に使う Claude Code も、`claude -p` で CI に組み込んだ自動実行も、Agent SDK で常時稼働させているバッチも、**同じサブスクリプション枠**から賄われていました。6月15日以降は、このうち**「プログラム的（エージェント的）に動かす利用」だけが分離**され、独立したクレジットプール（API 従量レート課金）に移りました。
...
>
> **運用判断**: CI を止めたくないなら usage credits を**ON**（ただしコストは青天井になりうる）、コスト暴走を防ぎたいなら**OFF**（ただし枠を超えると自動処理が止まる）。6/15 は施行済みのため、**未設定なら早急に**どちらを選ぶか決めてください。
...
## 9. 対応チェックリスト（施行済み・未対応なら今すぐ）

> 6/15 は施行済みです。以下は「まだ対応していない場合に、今からでも実施すべきこと」として読んでください。
...
- まずは**自動実行の棚卸し**と**クレジットの claim**を（**施行済み**のため、未対応なら今すぐ）。
