...
category: "Claude技術解説"
tags: ["Claude", "Opus 4.7", "Anthropic", "AIモデル", "リリースノート", "Fast mode"]
excerpt: "2026年4月16日にGAとなったClaude Opus 4.7の全体像。1Mトークン無料提供、xhigh effortレベル、画像3倍解像度、Adaptive Thinking、Amazon Bedrock / Google Cloud Vertex AI / Microsoft Foundry のマルチクラウド正式提供、2026年5月の Fast mode（research preview、2.5倍速・$30/$150 per MTok）まで主要機能を整理。"
draft: false
---
...
### 2.6 Fast mode（research preview、2026-05）

**Fast mode** は Opus 4.7 / 4.6 を **2.5 倍速** で動かせる高速設定です。2026年5月12日（PT）頃に research preview として公開され、**5月14日（PT）以降は Claude Code v2.1.142 で Opus 4.7 がデフォルト fast mode モデル**に昇格しました。

| 項目 | 内容 |
|---|---|
| 対象モデル | **Opus 4.7 / Opus 4.6 のみ**（Sonnet / Haiku / 他モデルは非対応） |
| 速度 | 標準 Opus の **約 2.5 倍** |
| 料金 | 入力 **$30** / 出力 **$150** per MTok（**標準 Opus の 6 倍**、1M コンテキスト窓全域でフラット） |
| 仕組み | 別モデルではなく、Opus の **API 設定をスループット優先に切替**。品質・能力は通常 Opus と同一 |
| 必要バージョン | Claude Code **v2.1.36 以降**。デフォルト Opus 4.7 化は **v2.1.142 以降** |
| 切替 | `/fast` を Tab で toggle、または `~/.claude/settings.json` で `"fastMode": true` |
| 4.6 にピン留め | `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE=1` を設定 |
| 利用条件 | **Usage credits 必須**（プランの含まれる利用枠ではなく credits から消費）。Team/Enterprise は **管理者の明示有効化**が必要 |
| 利用可能プラン | Pro / Max / Team / Enterprise / Claude Console |
| **非対応プラットフォーム** | Amazon Bedrock / Google Vertex AI / Microsoft Foundry では利用不可（Anthropic Console API と Claude サブスクのみ） |

#### 使い分け

| シーン | 推奨 |
|---|---|
| 対話的なコード反復・ライブデバッグ・タイトな締切 | **Fast mode** |
| 長時間の自律タスク・CI/CD・バッチ・コスト最優先 | 標準モード |
| 単純タスクで最速を狙う | Fast mode + 低 effort の組み合わせ |

#### コスト最適化の注意

会話の途中で fast mode に切り替えると、**それまでに溜まったコンテキスト全体に fast mode の uncached input 価格が適用**されるため、最初から有効化するほうが結果的に安くなります。

#### Rate limit 挙動

Opus 4.7 / 4.6 の fast mode は**同一の rate limit プール**を共有します。上限に達すると:

1. 自動で標準速度・標準価格にフォールバック
2. プロンプト横の `↯` アイコンがグレー化
3. クールダウン明けで fast mode が自動再有効化

組織で `fastModePerSessionOptIn: true` を managed settings に設定すれば、セッション開始時には毎回 OFF 状態となり、ユーザーは `/fast` で明示有効化する運用にできます。並列セッションのコスト暴走対策として有効です。

> **【2026-06-27追記】Opus 4.7 の fast mode は廃止予定（2026-07-24 removal）**
> 公式は **2026年6月25日（PT）に Opus 4.7 の fast mode を deprecated** とし、**2026年7月24日に removal（削除）** すると発表しました。removal 後は `claude-opus-4-7` に `speed: "fast"` を指定すると**エラー**になります（Opus 4.6 と異なり standard 速度へ自動フォールバックしません）。**モデル自体は standard 速度で引き続き利用可能**です。fast mode を続けたい場合は **Opus 4.8 の fast mode へ移行**してください（fast mode 価格は Opus 4.8 が **$10 / $50** per MTok、Opus 4.7 が $30 / $150 per MTok）。なお Opus 4.8 の fast mode は **Claude API・Claude Managed Agents のみ**で、Amazon Bedrock / Google Cloud / Microsoft Foundry では非対応です。出典: [Fast mode 公式ドキュメント](https://platform.claude.com/docs/en/build-with-claude/fast-mode)。
...
## 8.5. 後継モデル: Claude Opus 4.8（2026-05-28 PT / 2026-05-29 JST GA）

Opus 4.7 GA から **41日後**、後継となる **Claude Opus 4.8** が一般提供開始されました。価格・コンテキスト窓は据え置きのまま、ベンチマーク・Fast mode 価格・新機能で大きく前進しています。

### 主要スペック差分

| 項目 | Opus 4.7 | **Opus 4.8** |
|------|----------|-------------|
| API モデル ID | `claude-opus-4-7` | **`claude-opus-4-8`** |
| 価格（標準） | $5 / $25 per MTok | **$5 / $25 per MTok（据え置き）** |
| **Fast mode 価格** | $30 / $150 per MTok | **$10 / $50 per MTok（約 3倍安）** |
| コーディング（ベンチマーク） | 64.3% | **69.2%** |
| マルチ分野推論 | 54.7% | **57.9%** |
| 知識作業スコア | 1753 | **1890** |
| **Effort Control** | `/effort low / standard / high / xhigh` | **`low / default / extra / max` スライダー** (claude.ai) / `/effort` (Claude Code) |
| Dynamic Workflows 統合 | 非対応 | **対応**（Claude Code Enterprise / Team / Max。最大1000サブエージェント） |
| Messages API キャッシュ | system 入れ替えで invalidate | **タスク途中の system 更新がキャッシュ維持** |
| アライメント | xhigh で強化 | **近 Mythos Preview レベル**。コード欠陥見落としが約 **1/4** |

### Claude Code との関係

- v2.1.154（2026-05-28 PT）で **`claude-opus-4-8` がデフォルトモデル** に昇格
- `/effort xhigh` が高難度タスク向けデフォルト
- 既存ユーザーの個別 `/model` 設定は保持

### 移行判断

- **コスト**: 標準レート据え置き / Fast mode は約 3倍安 → **据え置きで性能上昇**
- **互換性**: モデル ID 切替のみ。Messages API のブレイキングチェンジは Opus 4.7 → 4.8 では新規発生なし（Opus 4.6 → 4.7 のブレイキングチェンジは継続適用）
- **推奨対応**: Opus 4.7 利用中の Production ワークロードは、A/B での検証後 `claude-opus-4-8` への切替が標準的な選択肢

→ 詳細は [**Claude Opus 4.8 完全ガイド**](/mdTechKnowledge/blog/claude-opus-4-8-guide/) を参照してください。本記事は **Opus 4.7 リファレンス** として保持されます。
