...
> **🔗 関連記事との「系統数」の違い**：本記事は**サブエージェントを含むエージェント全般の5系統**で整理します。一方、複数エージェントを動かす **"機構" の4系統**（Dynamic Workflows / Agent View / Managed Agents / Agent Teams）に絞った比較は [Claude のマルチエージェント4系統 — Agent View・Managed Agents・Agent Teams との比較](/mdTechKnowledge/blog/claude-code-agent-view-guide/) を参照してください（サブエージェント単体は Dynamic Workflows が動かす実体として扱うため、4系統側では独立系統に数えていません）。観点が違うだけで内容は矛盾しません。
