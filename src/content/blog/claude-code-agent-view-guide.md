...
date: 2026-05-31
category: "Claude技術解説"
tags: ["Claude Code", "Dynamic Workflows", "Agent View", "Managed Agents", "マルチエージェント", "オーケストレーション", "Research Preview"]
excerpt: "Claude の「マルチエージェント」は Dynamic Workflows（実行エンジン）・Agent View（管理UI）・Managed Agents（クラウド基盤）の3系統が混同されがち。Dynamic Workflows を起点に三者の役割・動く場所・確認コマンドを比較し、使い分けの判断軸を示す。後半に Agent View の起動・ステータス・キーバインド・自動化の詳細リファレンスを補足。"
draft: false
---
...
| 深掘り記事 | [DW 完全ガイド](/mdTechKnowledge/blog/claude-code-dynamic-workflows-guide/) | 本記事（後半 補足） | [Managed Agents ガイド](/mdTechKnowledge/blog/claude-managed-agents-guide/) |
