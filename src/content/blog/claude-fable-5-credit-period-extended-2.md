...
excerpt: "2026年7月7日で終わるはずだった Claude Fable 5 の『サブスク内利用（週間上限の最大50%まで追加費用なし）』が、7月12日→7月19日と2度目の延長を迎えた（Anthropic公式）。同時期の7月9日、OpenAI が GPT-5.6（Luna/Terra/Sol）を公開し、最上位 Sol はコーディングベンチで Fable 5 をわずかに上回りながら価格は50〜60%安いと報じられている。2度目の延長の中身、GPT-5.6 Sol との性能・価格比較、7/19までに取るべき行動を整理する。"
draft: false
---
...
> - **同じ7月9日、OpenAI が GPT-5.6（Luna / Terra / Sol の3段階）を公開**。最上位 **Sol はコーディングベンチマークで Fable 5 をわずかに上回りつつ、価格は50〜60%安い**（入力$5 / 出力$30 per MTok）と報じられている。
...
## 7月20日以降：Usage Credits の実務

**本記事公開時点（JST 2026-07-19）で、7/19（PT 23:59:59）が延長された無料枠の最終日**です。以降のことを、公式ヘルプで確認できる範囲で整理します。
...
- **Fable 5 の従量レート**: 入力 **$10** / 出力 **$50** per MTok。新トークナイザで同一文章のトークン数が約30%増える点も見積もりに織り込みます。

> 購入手続きの正確な導線（特にモバイルアプリからの購入可否）は変わりうるため、実際の有効化・購入は claude.ai の Settings → Usage と公式ヘルプで最新の手順を確認してください。

## 背景と今後

2度の延長は「容量（capacity）が整うまでの一時措置」という Anthropic の説明と整合的ではありますが、**GPT-5.6 Sol という価格競争力のある代替が登場した直後というタイミング**は、今後のFable 5の価格戦略を占う上で注視に値します。そして迎えた **本日 7/19（PT）が、その2度延長された期限の最終日**です。**本記事時点で3度目の延長は発表されておらず、予告どおり 7/20 から Usage Credits（従量課金）へ移行**する見込みです。無料で気軽に最上位モデルを試せる期間はここで一区切りとなり、以降は「本当に Fable 5 が要るタスクだけをクレジットで叩く」という費用対効果の判断に移ります。
