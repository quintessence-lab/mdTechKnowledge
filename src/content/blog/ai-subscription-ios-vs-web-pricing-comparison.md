...
## 8. 補足: Claude の Agent SDK 課金分離は「一時停止」された（2026-06-17 更新）

> **🚨 2026-06-17 更新**: この Agent SDK 課金分離は **施行予定日（2026年6月15日）当日に Anthropic が公式に一時停止（pause）** しました（公式サポート記事冒頭 *"For now, nothing has changed."*）。**現在 Agent SDK・`claude -p`・GitHub Actions は従来どおりサブスク枠から消費**され、独立クレジットプールは稼働していません。本記事の料金比較（インタラクティブ利用前提）への影響も**現状なし**です。白紙撤回ではなく形を変えた再開はありうるため、当初設計を以下に残します。詳細は [Claude Agent SDK / claude -p 課金分離ガイド](/mdTechKnowledge/blog/claude-agent-sdk-billing-split/) を参照。

Web/iOS 比較とは別軸ですが、当初の発表では Anthropic が **2026年6月15日（PT）に Claude サブスクリプションの課金プールを分割**する予定でした（プログラマティック利用＝Agent SDK を独立プールへ）。**この施行は当日に一時停止**されています。以下は当初設計（再開時の参考）です。
...
**分類**:
- **Agent SDK credits 消費**: Agent SDK（Python/TS の自プロジェクト）、`claude -p`、Claude Code GitHub Actions、第三者 Agent SDK アプリ
- **従来サブスク枠で継続**: Claude.ai Web/アプリのチャット、Claude Code 端末でのインタラクティブ実行、Claude Cowork

**枯渇時**: `usage credits` 有効化済なら API 標準レートで超過従量課金、未有効化ならリクエスト停止（次回月次リフレッシュまで）。

**Web vs iOS への影響**: 本記事の料金比較は **インタラクティブ利用前提** のため、Agent SDK を業務利用しないユーザーには影響しません。一方、Claude Code GitHub Actions や `claude -p` を多用するユーザーにとって、**月額 credits は Web 契約でしか opt-in できない可能性が高い**ため、契約経路としては Web の優位性がさらに強化されます（iOS 契約者は Web アカウントとの紐付け要確認）。

詳細は [Claude を契約するなら iOS と Web どちらが安いか](/mdTechKnowledge/blog/claude-ios-vs-web-pricing-comparison/) のセクション7、公式情報は [Use the Claude Agent SDK with your Claude plan — Claude Support](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan) を参照。

> **注**: Gemini / ChatGPT には対応する課金分離アナウンスは現時点（2026-05-20）でなく、本変更は **Claude 固有の論点** です。3社比較表自体は引き続き有効ですが、Claude 選択時の「Agent SDK の実質的価値追加」分を加味する余地があります。
