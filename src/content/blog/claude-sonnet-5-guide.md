...
| コンテキストウィンドウ | 1M トークン（**推測・要確認**） | 公式ニュースに明記なし。Sonnet 4.6 が 1M のため踏襲の見込み |
| 導入価格（〜08-31） | 入力 $2 / 出力 $10 per MTok | 期間限定 |
| 通常価格（09-01〜） | 入力 $3 / 出力 $15 per MTok | Sonnet 4.6 と同水準 |
| デフォルト先 | Free / Pro / Claude Code | 前世代から交代 |

> **コンテキストウィンドウについて（推測）:** 公式ニュースページには 1M トークンの記載が確認できませんでした。Sonnet 4.6 がすでに 1M（パブリックベータ）を提供していることから、Sonnet 5 も**ネイティブ 1M クラスを踏襲すると推測**されますが、正確な値は [Anthropic の公式ドキュメント](https://www.anthropic.com/claude/sonnet)での確認をおすすめします。
...
| Google Vertex AI | **coming soon（近日提供）** | 公式 |
