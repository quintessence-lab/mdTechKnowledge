...
Cowork のライブアーティファクト（8月19日より前）も同様に**動作は続くが、その場では編集できません**。編集したい場合は、**旧ライブアーティファクトを開き、「Share」から新しいアーティファクトとして再公開**します。ヘルプには、その手順について次のように書かれています。

> If you already shared the live artifact, the dialog shows its existing link. People with that link see the new version.
> （すでに共有していたライブアーティファクトなら、ダイアログに既存のリンクが表示される。そのリンクを持つ人は新しいバージョンを見る）

つまり、**既存リンクが新バージョンを指すのは、この「Share から再公開」を使ったとき**です。

### 実例: 別の方法で作り直すと、リンクは引き継がれない

ある利用者が、7月に作った Cowork のライブアーティファクト（ダッシュボード）を Windows のデスクトップアプリから更新しようとしたところ、次のようになりました。

- 以前使えていたライブアーティファクトの更新用ツールが、そのセッションに見当たらなかった。デバイスも「未接続」と表示された。
- 代わりに、**標準のアーティファクト機構で新しいアーティファクトを別に作成**し、内容は同一のまま**新しい URL で公開**した。
- 新しく作ったものは**非公開**で始まった。他の人に見せるには、そのページの Share メニューで共有設定を変える必要がある。

この方法では、**旧ライブアーティファクトとその旧リンクは作り直されないため、古い内容のまま残る**と考えられます（ヘルプはこのケースを説明しておらず、上記の手順の記載からの読み取りです）。旧リンクを他の人に渡している場合は、新しい URL を案内し直すか、旧ライブアーティファクトを開いて Share から再公開する必要があります。

なお、更新用ツールが見当たらなかった**原因は確認できていません**。8月19日・9月16日の変更との関係は推測にとどまり、一次情報には記載がありません。
...
- **8月19日より前のライブアーティファクトを更新したいとき**は、旧ライブアーティファクトを開いて「Share」→ 新しいアーティファクトとして再公開、が既存リンクを引き継ぐ唯一の方法です。別に新規作成すると新しい URL・非公開になり、旧リンクは古い内容のままです（前節の実例を参照）。
