...
| **MCP tunnels（Research Preview）** | **社内プライベートネットワーク上の MCP サーバー**へ、パブリックエンドポイントを公開せずアクセス。軽量ゲートウェイが**アウトバウンド1本の暗号化接続**を Anthropic 側へ確立する。**【2026-06-22】管理 API が Admin API（`/v1/organizations/tunnels`）→ Claude API（`/v1/tunnels`）へ移管**（`anthropic-beta: mcp-tunnels-2026-06-22` ヘッダー・`workspace:manage_tunnels` WIF スコープ。旧側も移行期間中は有効） | 「3.4 到達可能なネットワーク」の制約（外部から内部 MCP に届かない）を、インバウンド開放なしで解消 |
...
## 13. MCP 2026-07-28 仕様（正式リリース） — ステートレス化が接続パターンに与える影響

> **【2026-07-28 追記】正式リリース確定**: 2026年5月21日（PT）にロックされた Release Candidate は、予定通り **2026年7月28日（PT）に正式仕様として公開**されました（[MCP公式ブログ](https://blog.modelcontextprotocol.io/posts/2026-07-28/)）。RC からの内容変更は公式発表に言及がなく、以下の解説はそのまま正式仕様として有効です。

2026年5月21日（PT）にロックされた MCP の新仕様では、**Streamable HTTP トランスポートがステートレス化**される方針が確定しました。本記事の3パターンすべての**「2回目以降のリクエストの振る舞い」**が変わるため、運用設計に影響が及びます。

### 13.1 何が変わるか

| 観点 | 変更前（〜2026-07-27） | 変更後（2026-07-28 仕様適用後） |
|---|---|---|
...
| **パターン2: Claude Code（端末）** | 中 | `mcp.json` の HTTP MCP 設定は同じだが、**自社運用 MCP サーバー側の新仕様対応が必須**。Mcp-Session-Id を発行する旧実装は移行作業を伴う |
| **パターン3: Claude Code on the Web** | 小（透過的） | サンドボックス内の MCP クライアントが自動追従。サーバー側の新仕様対応が前提 |
...
- [ ] SDK メンテナー・実装者向けに設けられた**10週間の検証ウィンドウ**（RCロック2026-05-21〜正式公開2026-07-28）は既に終了。移行は先延ばしせず着手する

### 13.4 キャッシュとルーティングの新仕様

- **`tools/list` のキャッシュ（`ttlMs` / `cacheScope`・SEP-2549）**: list 系・resource read の応答に、HTTP の `Cache-Control` を模した **`ttlMs`（鮮度の有効期間）** と **`cacheScope`（ユーザー間で共有可否）** が付与されます。クライアントは「`tools/list` の結果を**いつまでキャッシュしてよいか・ユーザー横断で共有してよいか**」を明示的に判断でき、ステートレスでも**毎回 list を取り直す無駄**を避けられます。
- **`Mcp-Method` ヘッダーによるルーティング（SEP-2243）**: リクエストに **`Mcp-Method`**（例: `Mcp-Method: tools/call`）と **`Mcp-Name`**（例: `Mcp-Name: search`）ヘッダーを必須化。**LB・ゲートウェイ・レートリミッタが本文（body）を覗かずに操作単位でルーティング**できます。ステートレスでスティッキーセッションが無くても、「重い `tools/call` は専用プールへ」「`tools/list` はキャッシュ層へ」といった振り分けがヘッダーだけで可能になります。

> **接続パターンへの含意**: パターン2（自社運用 MCP サーバー）では、ステートレス化に合わせて **`Mcp-Method` ベースのゲートウェイ設計**と **`ttlMs` を返す list 応答**を用意すると、ラウンドロビン LB のまま効率よくスケールできます。パターン1・3（Anthropic 側クライアント）は透過的に追従します。

詳細は [MCP (Model Context Protocol) アーキテクチャ詳細](/blog/mcp-architecture/) の「MCP 2026-07-28 仕様」節を参照。

---

## 14. 【2026-08-26 追記】MCP 新ロードマップ（2026-08-22 公開）— 接続パターンの今後

2026-07-28 仕様の正式リリースを受け、MCP 公式ブログが**新しいロードマップ**を公開しました（前回2026年3月版から約5ヶ月ぶり）。**5つの優先領域**のうち、本記事の接続パターンに直接効いてくるのは次の3つです。

| 優先領域 | 接続パターンへの影響 |
|---|---|
| **HTTP-native transport の全面化** | 2026-07-28 仕様で実現したリモートサーバーの標準 HTTP ワークロード化を、**ローカルサーバーを含む全展開モードへ拡張**する方針。実現すれば、本記事の「stdio（ローカル）／Streamable HTTP（リモート）」という第1章の二分法が **HTTP に一本化**される方向で、パターン2（Claude Code ローカル）の設定・デバッグ手法も HTTP ベースへ寄っていく |
| **Agent Identity / エンタープライズセキュリティ** | 現行の認可は「**人がアクセスを承認する**」前提（第6章の OAuth 前提設計）。これを超えて、**DPoP**（所持証明）の採用と **Workload Identity Federation** の定義、IETF（OAuth 標準団体）との連携が進む。エージェントが人の同席なしに動くパターン3・4（Web／Managed Agents）で、**エージェント自身のアイデンティティに基づく認可**が可能になる方向 |
| **Agentic Messaging Primitives** | リクエスト／レスポンス型を超える**サーバー発信イベント（webhook・channel）**や購読・進捗通知の統合。長時間タスクをポーリングなしで扱えるようになり、パターン4（MCP Tunnels / Managed Agents）の長時間セッション運用が素直になる |

残る2領域は**ツールプリミティブの改善**（`tools/call` 結果処理の標準化・大規模サーバー向けの段階的発見）と **SDK 開発者体験**（準拠性テスト・ドキュメント精度）。また、優先領域に該当する SEP は**加速レビュー**の対象になることも明文化されました。
