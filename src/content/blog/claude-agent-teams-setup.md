...
excerpt: "Claude Code Agent Teams（ローカル/tmuxで動くマルチエージェント機能）の有効化・表示モード・チーム管理を解説。【2026-06更新】v2.1.178 で内部モデルが大改訂され、TeamCreate/TeamDelete ツール廃止・各セッションが暗黙の1チームを持ち Agent ツールの name でチームメイトを直接スポーンする方式へ。クラウド側 Managed Agents の Multi-agent orchestration との位置づけや使い分け判断ガイドも掲載。"
draft: false
---
...
> **【2026-06 追記】v2.1.178（2026-06-15 PT）で内部モデルが大改訂**: `TeamCreate` / `TeamDelete` ツールが**廃止**され、`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 有効時は**各セッションが最初から暗黙の1チームを持つ**モデルになりました。**事前のチーム作成ステップは不要**で、`Agent` ツールの **`name` パラメータ**を指定すると直接チームメイトをスポーンできます。下記の自然言語での「チームを作って」という指示は引き続き有効です（Claude が内部で `Agent(name:...)` を発行）が、明示的な `TeamCreate`/`TeamDelete` 呼び出しは行われなくなりました。あわせて v2.1.178 では権限ルールに `Tool(param:value)` 構文が追加され、例えば `Agent(model:opus)` で特定モデルのチームメイト/サブエージェント起動をブロックできます（→ [バージョン履歴](/mdTechKnowledge/blog/claude-code-version-history/)）。
...
>
> **v2.1.178 以降**: `TeamDelete` ツールは廃止されました。暗黙チームモデルでは「解散」は各チームメイトのシャットダウン（上記）で行い、セッション終了時に暗黙チームも消えます。自然言語での「チームをクリーンアップして」は引き続き有効です。
...
| **Multi-agent orchestration** | なし（Agent Teamsがローカル実装） | ✓ パブリックベータ（2026年5月〜）。リードエージェント＋最大20専門エージェント並列、共有FS |
...
### 2026年4〜5月の Managed Agents 動向との関係

Managed Agents 側では2026年4〜5月に以下の動きがありました（詳細は別記事 [Claude Managed Agents 簡易ガイド](/mdTechKnowledge/blog/claude-managed-agents-guide) を参照）。
...
| **2026-05-06** | **Multi-agent orchestration パブリックベータ開始**（Code with Claude SF イベント） | Managed Agents 側に **クラウドネイティブなマルチエージェント機構** が追加。Agent Teams（ローカル/tmux）とは実行レイヤーが異なる。詳細は下記 |

---

### 2026年5月: Multi-agent orchestration — リードエージェント＋専門エージェント並列処理

2026年5月6日（PT）のCode with Claude SFイベントで発表された **Multi-agent orchestration** は、Managed Agentsに組み込まれたクラウドネイティブなマルチエージェント機構です。
...
├──────────┬──────────┬──────────┬──────────┬──────────────────┤
│ 専門①   │ 専門②   │ 専門③   │ 専門④   │  … 最大20並列    │
│ 独自モデル│ 独自モデル│ 独自モデル│ 独自モデル│                  │
│ 独自プロンプト│ 独自プロンプト│ ...  │          │                  │
│ 独自ツール│          │          │          │                  │
├──────────┴──────────┴──────────┴──────────┴──────────────────┤
│  共有ファイルシステム（全エージェントが読み書き可能）            │
│  永続イベントログ（各エージェントが過去の行動を記憶）           │
└──────────────────────────────────────────────────────────────┘
```
...
|------|------|
| 最大並列専門エージェント数 | **20** |
| 各専門エージェントの独立性 | 独自のモデル・システムプロンプト・ツールセットを持つ |
| 共有インフラ | 共有ファイルシステム＋永続イベントログ |
| リードエージェントの介入 | ワークフロー途中でも専門エージェントに確認・修正指示が可能 |
| 提供形態 | パブリックベータ（2026年5月〜） |

#### Netflix での導入事例

Netflix はプラットフォームチームで Multi-agent orchestration を既に本番投入しています。リードエージェントがコードベース分析・テスト生成・ドキュメント更新などを各専門エージェントに分担させ、CI/CDパイプラインに統合した形で稼働しています。

#### Agent Teams との違い

| 比較 | Agent Teams（Claude Code） | Multi-agent orchestration（Managed Agents） |
|------|--------------------------|---------------------------------------------|
| 実行場所 | ローカルマシン / tmux | Anthropicクラウドサンドボックス |
| 専門エージェント数 | 制限なし（実質3〜5が上限） | 最大20並列 |
| 永続化 | なし（セッション終了で消滅） | ✓ イベント永続化・状態復元可能 |
| 設定方法 | 自然言語で口頭指示 | API経由でモデル/プロンプト/ツールを明示設定 |
| セットアップコスト | 環境変数1行 | Managed Agents オンボーディング必要 |
| 料金 | APIトークン課金のみ | トークン + $0.08/session-hour |
...
#### Managed Agents（Multi-agent orchestration含む）が適しているケース

- 顧客向けプロダクトに **エージェント機能を組み込みたい**
- **最大20の専門エージェントを並列** に走らせる複雑ワークフローを本番運用したい
...
- **Netflix のように** コードベース分析・テスト生成・ドキュメント更新を完全自動化したい
...
- **本番フェーズ**: 同じロジックを Managed Agents（Multi-agent orchestration）に移植してクラウドデプロイ
...
- [9to5Mac: Anthropic updates Claude Managed Agents with three new features（2026/5/7）](https://9to5mac.com/2026/05/07/anthropic-updates-claude-managed-agents-with-three-new-features/)
