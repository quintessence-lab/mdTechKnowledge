...
excerpt: "Claude Code Agent Teamsの有効化からtmux/in-processの2つの表示モード、チーム作成・管理方法までを解説。Claude Managed Agentsとの違いも整理。"
draft: false
---
...
## Claude Managed Agents との位置づけ（2026年4月時点）

2026年4月8日にパブリックベータとして提供開始された「**Claude Managed Agents**」は、Agent Teamsとは**別製品**である。混同しやすいため、以下に違いを整理する。

| 項目 | Agent Teams | Claude Managed Agents |
|------|-------------|----------------------|
| **種別** | Claude Code の実験的機能（CLI） | 独立した API 製品 |
| **利用方法** | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` で有効化 | API（`managed-agents-2026-04-01` ベータヘッダー必須） |
| **対象ユースケース** | 開発セッション内の協調作業（コードレビュー・調査） | 本番向け長時間実行エージェントパイプライン |
| **課金** | Claude Code プランに含まれる | Claude API トークン料金 + $0.08/session-hour |
| **アーキテクチャ** | tmux / in-process の2表示モード | Session / Harness / Sandbox の3層独立管理 |
| **Memory** | 非対応 | パブリックベータ（2026年4月下旬〜） |
| **エンタープライズ機能** | なし | RBAC・グループ支出制限・OpenTelemetry 対応 |

**Agent Teams** は Claude Code CLI の中でインタラクティブに複数インスタンスを協調させる機能で、開発者がターミナル上で即座に使えるのが特徴。**Claude Managed Agents** はプログラムから API で呼び出し、スケーラブルな本番エージェントを構築するための製品である。
...
- [Claude Managed Agents 簡易ガイド](/mdTechKnowledge/blog/claude-managed-agents-guide)
