...
> **🚨 2026-06-17 更新**: この課金プール分離は **施行予定日（2026年6月15日）当日に Anthropic が公式に一時停止（pause）** しました（公式サポート記事冒頭 *"For now, nothing has changed."*）。**現在は『インタラクティブ／プログラマティック』の課金プール分離は行われておらず、すべて従来どおりサブスクリプション枠から消費**されます。慌てた対応は不要です。ただし白紙撤回ではなく、**形を変えて再開する可能性**があります（発効前に改めて告知予定）。詳細・最新状況は [Claude Agent SDK / claude -p 課金分離ガイド](/mdTechKnowledge/blog/claude-agent-sdk-billing-split/) を参照。以下は**当初発表されていた（現在は停止中の）設計**です。
