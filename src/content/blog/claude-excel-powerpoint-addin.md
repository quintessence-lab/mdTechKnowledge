...
## 2026年5月中旬のアップデート（Word GA・Outlook パブリックベータ・Claude for Small Business）

2026年5月7日（PT）に Microsoft 365 統合の節目となるアップデートが続けて公開され、5月13日（PT／14日 JST）には小規模事業者向けの新パッケージ **Claude for Small Business** が発表された。

### Claude for Word が GA（一般提供開始）

2026年4月13日に投入されていた **Claude for Word** が **GA（一般提供）** に到達した。これにより Excel・PowerPoint と並ぶ正式提供アプリとなり、3アプリ間で会話コンテキストを引き継ぐ「Office アドイン三本柱」が確立した。

| アプリ | 提供状況（2026年5月時点） |
|---|---|
| Claude for Excel | GA（2026年1月24日 Pro 展開） |
| Claude for PowerPoint | GA（2026年2月20日 Pro 展開） |
| Claude for Word | **GA（2026年5月7日）** |
| Claude for Outlook | **パブリックベータ（2026年5月12日頃）** |

### Outlook パブリックベータへ移行

これまで「coming soon」として案内されていた **Claude for Outlook** が、2026年5月12日頃に **有料プラン向けパブリックベータ** として公開された。Excel→PowerPoint→Word→Outlook の4アプリで会話コンテキストが共有され、メール返信下書き・会議準備資料生成・Excel データ参照付きメール送付などの横断ワークフローが可能になる。

> "most work doesn't live in one application, so Claude moves between them as you do."（Anthropic リリースノート）

#### 4アプリ横断ワークフローの典型例

| シーン | 流れ |
|---|---|
| 月次レポート作成 | **Excel** で売上集計を Claude に分析させる → **PowerPoint** に開いて同じ会話を継続、自動でスライド化 → **Outlook** に切替えて経営陣宛のサマリメールを下書き |
| 提案書ドラフト | **Word** で骨子を Claude と作成 → **Excel** に切替えて費用見積もり表を Claude に生成させる → **PowerPoint** で顧客プレゼン用スライド化 → **Outlook** で送付メール文面作成 |
| 商談フォローアップ | **Outlook** で受信メールから論点抽出 → **Word** で議事録ドラフト → **PowerPoint** で次回提案案 → 全工程で同じ会話文脈を保持 |

会話文脈は **Microsoft 365 アカウント単位** で同期され、アプリ間切替時に「先ほどの売上データを参照して…」のような後方参照が破綻しない点が、個別アプリ統合との大きな違い。

### Claude for Small Business（Cowork 連携・8コネクタ）

2026年5月13日（PT）に発表された **Claude for Small Business** は、Claude Cowork 上で SMB（中小企業）が日常利用する業務ツールへ AI を直接組み込むパッケージである。Microsoft 365（Excel / Word / PowerPoint / Outlook）を含む計 **8 コネクタ**との連携、**15 本の自動化ワークフロー**、**15 スキル**を備える。
...
Claude for Small Business については [Claude Cowork アップデートまとめ](/blog/claude-cowork-updates/) の SMB セクションで詳述する。本稿の Excel・PowerPoint アドインから見たポイントは、**SMB 向け Cowork 経由でも Excel・PowerPoint・Word・Outlook データに同等の文脈アクセス**ができるようになり、アドイン経由とエージェント経由の双方で M365 データを扱える選択肢が整った点である。

---

## 2026年7月のアップデート（M365 コネクタに「書き込みツール」追加）

2026年7月7日（PT）、**Claude Enterprise の Microsoft 365 コネクタに「書き込み（Write）ツール」が追加**されました。従来は M365 データの**参照（読み取り）中心**でしたが、Claude が M365 上で**実際にアクションを実行**できるようになります。

> **注意（サーフェスの違い）**: これは Office 上で動く**アドイン（Excel/PowerPoint/Word/Outlook プラグイン）そのものの機能ではなく、Claude Enterprise の「Microsoft 365 コネクタ」**（Cowork/エージェントから M365 を操作する経路）の拡張です。アドインと同じ M365 連携の系譜ですが、提供面が異なります。

### 追加された書き込みアクション

| 領域 | できること |
|:---|:---|
| **メール** | 下書き作成・**送信**・整理・下書き管理（ゼロから作成／指示に基づく作成 → アカウントから直接送信） |
| **カレンダー** | イベントの**作成・更新・削除**、会議設定、リマインダー、繰り返し予定の管理 |
| **メールボックス設定** | 受信ルール・署名・メールボックス構成の更新 |
| **OneDrive / SharePoint** | ファイルの**新規作成・既存更新**（Word / Excel / PowerPoint 文書を含む） |
| **Teams** | **読み取りのみ**（メッセージ・チャネルの参照は可能だが、**書き込みは未対応**） |

### 有効化には2段階の管理者承認が必要（既定オフ）

書き込みは強力なため、**2層の承認**が要求されます。

1. **Microsoft Entra の管理者**が、更新された権限セットに**同意（consent）**する。
...
| **会話コンテキスト共有** | Excel⇔PowerPoint⇔Word⇔Outlook間で共有（Outlookは2026-05-12頃パブリックベータ） | Work IQが Outlook / Teams / Excel / OneDrive / SharePoint を横断 |
| **横断範囲** | Excel・PowerPoint・Word・Outlook（Outlookはパブリックベータ） | M365全アプリ＋Microsoft Graph全体 |
...
| **コンテキスト共有の境界** | Outlookは2026-05-12頃パブリックベータで対応済み。他Microsoftアプリへの拡張は今後予定 |
...
- **Outlook パブリックベータ（2026年5月12日頃〜）**: Excel・PowerPoint・Word に続く4つ目の M365 アプリ統合がパブリックベータに到達。Word→Outlook 間の会話コンテキスト引き継ぎを含む4アプリ横断統合が完成
