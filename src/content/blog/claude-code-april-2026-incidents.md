...
category: "Claude技術解説"
tags: ["Claude Code", "Anthropic", "ポストモーテム", "プラン", "品質問題", "リリース管理", "レート制限", "SpaceX"]
excerpt: "2026年4月後半に発生した2つの事案（Proプラン除外騒動・品質低下ポストモーテム）の経緯と教訓を整理。2026年5月のSpaceX Colossus 1契約によるClaude Code制限改善（5時間制限倍増・ピーク時制限廃止）も後日談として収録。"
draft: false
---
...
### 4月騒動への「後日談」: SpaceX Colossus 1契約とレート制限改善

2026年5月6日（PT）、Anthropicは**SpaceX傘下のxAIが運営するColossus 1データセンター（Memphis, TN）の全計算能力を確保**する契約を発表した。300MW超・NVIDIA GPU 22万台超の追加コンピュートがAnthropicに供給されることで、4月の騒動で問題の根本にあった「容量逼迫」が直接解消される形となった。

この発表と同日（2026年5月6日）に、Claude Codeおよびプラン全体に以下の制限緩和が実施された:

| 変更項目 | 変更内容 | 対象プラン |
|---|---|---|
| Claude Code 5時間レート制限 | **倍増** | Pro / Max / Team / Enterprise |
| Claude Code ピーク時間帯の制限 | **廃止** | Pro / Max |
| Claude Opus API レート制限 | 大幅引き上げ | 全API利用者 |

4月21〜22日の騒動では、Anthropicが「容量制約からPro向けClaude Codeを絞り始める」可能性が懸念された。今回の改善措置は、騒動当時に「今後は加入者に影響する変更は事前通知する」と約束したことと合わせ、**4月コミットメントの具体的な履行**として受け取られている。

特に「ピーク時間帯の制限廃止」はProユーザーが最も不満を感じていた部分（夕方〜夜間の応答速度低下）に直接応えるものであり、前述の「Proプランの位置付け見直し」への懸念を和らげる効果がある。

詳細は [Anthropic コンピュートインフラ & TPUパートナーシップ — SpaceX xAI Colossus 1章](/blog/anthropic-tpu-compute-partnership) を参照。
