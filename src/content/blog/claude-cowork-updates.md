...
### 2026年6月：Cowork 5時間レート制限2倍キャンペーン（期間限定）

| 日付 | 内容 |
|------|------|
| 6月5日（PT）/ 6月6日（JST） | **Cowork 5時間レート制限2倍キャンペーン開始（〜7月5日）** ― Anthropic の Boris Cherny（Claude Code 統括）がソーシャル投稿で告知（正式なニュースルーム記事は未公開）。**Cowork の5時間レート制限ウィンドウのみを2倍**に拡大する約1か月間の期間限定措置で、**週次の利用上限は変更なし**。対象は **Pro / Max / Team**（およびレガシー seat-based Enterprise）で、**無料プラン・従量制（usage-based）Enterprise は対象外**。**→ 延長の発表はなく、予定どおり2026年7月5日で終了**（以降、5時間枠は標準水準に復帰） |
| 6月12日 | **インライン編集（ドラフトのその場編集）** ― Claude が作成したドラフト（レポート・計画書・ブリーフ等）がチャット横に開き、**変更したい箇所をハイライトして指示を入力するだけで、その位置にピンポイントで編集が反映**されるように。チャットで改めて修正内容を説明し直す必要がなくなり、Cowork でのドキュメント反復作業が短縮。※Cowork 専用ではなく**チャットのドラフト（Artifacts）にも共通**の機能 |

> **補足**: Claude のレート制限は Claude Code・Claude.ai チャット・Cowork の各サーフェスで**共有プール**として消費される。本キャンペーンで2倍になるのは **Cowork の5時間ウィンドウのみ**で、Claude Code / Claude.ai 側は通常の上限のまま。週次上限も据え置きのため、Cowork を集中的に使っても週次予算は他サーフェスと合算で消費される点に注意。

**参考**: [The New Stack: Anthropic Claude Cowork promotion（2026/6）](https://thenewstack.io/anthropic-claude-cowork-promotion/) ／ [Mervin Praison: Anthropic Doubles Claude Cowork 5-Hour Limits Through July 2026（2026/6）](https://mer.vin/2026/06/anthropic-doubles-claude-cowork-5-hour-limits-through-july-2026/)

### 2026年6月18日：エンタープライズ向けコネクタ一括認可（EMA／Okta）— Cowork も対象

| 日付 | 内容 |
|------|------|
| 6月18日（PT） | **MCP Enterprise-Managed Authorization（EMA）ベータ提供開始** ― エンタープライズの管理者が IdP（最初の対応は **Okta**、Cross App Access を利用）でコネクタを一度承認すれば、従業員は既存グループ／ロールに応じて**初回ログイン時にゼロタッチでコネクタが自動付与**される（アプリごとの個別 OAuth 同意画面が不要）。この認可は Anthropic の共有 MCP レイヤーに実装されるため、**Claude チャット／Claude Code／Cowork を横断して一貫適用**される。 |

> **Cowork から見た意味**: これまで Cowork でエンタープライズのコネクタ（SaaS）を使うには各ユーザーが個別認可する必要があったが、EMA により**管理者の一括プロビジョニング**で Cowork 上のコネクタ利用が即時に整う。ローンチ時の対応コネクタは Asana / Atlassian / Canva / Figma / Granola / Linear / Supabase の7種（Slack ほか追加中）。なお現時点は **GA ではなくベータ**（Team / Enterprise 向け）である点に注意。技術詳細は [MCP アーキテクチャ詳細](/mdTechKnowledge/blog/mcp-architecture/) の「Enterprise-Managed Authorization（EMA）」節を参照。

**参考**: [Enterprise-Managed Authorization（MCP公式ブログ）](https://blog.modelcontextprotocol.io/posts/enterprise-managed-auth/) ／ [Anthropic: Enterprise-managed connector authorization](https://claude.com/blog/enterprise-managed-auth) ／ [Okta: Featured identity provider for Claude](https://www.okta.com/en-ca/newsroom/press-releases/okta-becomes-a-featured-identity-provider-powering-secure-ai-agent-connections-for-claude-enterprise/)

### 2026年7月7日：Cowork が Web・iOS・Android へ展開＋「リモートセッション」

| 日付 | 内容 |
|------|------|
| 7月7日（PT） | **Cowork が Web / iOS（iPhone・iPad）/ Android へ展開**（従来はデスクトップアプリ中心）。**Max プランからベータで数週間かけて順次ロールアウト**し、以降のプランへ拡大予定。アクセスは Claude アプリのサイドバー（モバイル）と claude.ai から。 |

この展開の目玉が **「リモートセッション（Remote sessions）」** です。従来 Cowork はローカルPC上でのエージェント実行が前提でしたが、**セッションがクラウド側で継続**するようになりました。

- **PC を閉じても作業が継続**: 「ノートPCを閉じて会議に向かっても、Claude は動き続ける」。デスクで始めたタスクを**スマホから進捗確認**し、**完成物をどのデバイスからでも受け取れる**（クロスデバイス継続）。
- **デバイス不要のスケジュール実行**: 端末がオンラインでなくても、指定時刻のタスクをクラウドで自律実行できる。
- **承認が必要な場面は通知**: 「あなたにしか判断できない呼び出し（承認）に達すると、その問いが**スマホに届く**」。応答後にセッションが再開する。
- **利用上限の2倍を 2026年8月5日まで延長**: Cowork の利用上限を **8/5 まで2倍**に拡大（6月から続く増枠措置の延長）。
