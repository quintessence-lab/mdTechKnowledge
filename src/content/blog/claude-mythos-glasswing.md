...
excerpt: "Anthropicがセキュリティタスク特化型LM「Claude Mythos Preview」と重要インフラ防衛プロジェクト「Project Glasswing」を発表。能力範囲・想定ユースケース・公開条件・既存Claudeとの差別化に加え、AWS Bedrock・Google Vertex AIでのGated Research Preview提供、GlasswingからClaude Security Beta（防御製品）への発展経緯、2026年6月の第2次拡張（約150組織・15カ国以上）と『数週間以内』の一般提供方針、そして初代 Mythos Preview の 2026-06-30 廃止予定（後継 Mythos 5 が輸出管理で停止中という矛盾）までを整理。"
draft: false
---
...
> Mythos Preview で「公開しない」とした当初方針からの転換が、**安全classifier＋Opus 4.8 フォールバック**という設計で「一般公開（Fable 5）」と「承認組織向け強化版（Mythos 5）」に分離する形で着地した、と言えます。Fable 5 単独の詳細は、別シリーズ [Claude Fable 5 徹底解剖①概要編](/blog/claude-fable-5-overview/)（全3話：概要／[ベンチマーク・性能](/blog/claude-fable-5-benchmarks/)／[安全設計・社会的文脈](/blog/claude-fable-5-safety-context/)）で詳しく解説しています。

### 【2026-06-13追記】公開3日で停止 — 米政府の輸出管理指令により Fable 5 / Mythos 5 が無効化

リリースからわずか3日後、事態は急転しました。**2026年6月12日 17:21（ET）＝6月13日 06:21（JST）**、Anthropic は米政府から**輸出管理指令（書簡）を受領**し、同日中に **Claude Fable 5 と Mythos 5 を全顧客向けに無効化**しました（[CNBC](https://www.cnbc.com/2026/06/12/anthropic-disables-access-to-fable-5-and-mythos-5-to-comply-with-government-directive.html) ほか）。本記事の中心テーマである **Mythos も停止対象**です。

| 項目 | 内容 |
|---|---|
| **対象モデル** | Claude Fable 5・Claude Mythos 5（**それ以外の全モデルは影響なし**。Opus 4.8 等は通常利用可） |
| **停止範囲** | 指令は「**米国内外を問わず、いかなる外国人のアクセスも停止せよ**」（外国籍の Anthropic 従業員を含む）。リアルタイムで外国人だけを選別できないため、確実に従う唯一の方法として **全顧客で一律無効化** |
| **原因（とされる）** | 「**コードベースを読ませてソフトウェアの欠陥を特定・修正させる**」狭いジェイルブレイク手法（攻防両用のデュアルユース）。ただし政府は具体的懸念を文書で示さず、口頭の証拠のみだったと報じられている |
| **法的根拠** | 国家安全保障を理由とした輸出管理指令。具体的な法令条項はどのソースも未特定（発出者は報道ベースで商務長官 Lutnick／米商務省、公式声明は「米政府」とのみ） |
| **解除見通し** | 公式は「**可能な限り早く復旧する**」のみで、期日・条件は未提示（「数週間」という観測は単一ソースで要注意） |

Anthropic は指令には従いつつ、その根拠には「**同じ能力は OpenAI の GPT-5.5 等にもある**」「数億人に展開した商用モデルを狭いジェイルブレイクの発見でリコールすべきでない」と反論し、これは誤解だとして早期復旧を目指す立場です。Mythos クラスを「危険なほど高性能」と打ち出した透明性が、かえって規制当局の精査を招いた皮肉な構図とも言えます。

> 本件の時系列・法的根拠・各社報道の突き合わせ（**確認済み事実／報道ベース／単一ソース**の区別）は、専用記事 [米政府、Claude Fable 5 と Mythos 5 を停止 — 公開3日での輸出管理指令](/blog/claude-fable-5-export-control-suspension/) で詳しく整理しています。

### 【2026-06-30 予定】Claude Mythos Preview が廃止（retire）— 後継 Mythos 5 は停止中という矛盾

Anthropic 公式の「Model deprecations」ページに、本記事の主役である初代 **Claude Mythos Preview（`claude-mythos-preview`）を 2026年6月30日に廃止（retire）する**ことが明記されました。Anthropic の用語定義では「retired」は**モデルが利用不可となりリクエストが失敗する状態**を指します（deprecation の先の最終段階）。

| 項目 | 内容 |
|---|---|
| **対象** | Claude Mythos Preview（`claude-mythos-preview`、招待制の研究プレビュー版） |
| **廃止日** | **2026年6月30日**（この日以降、`claude-mythos-preview` へのリクエストは失敗する） |
| **公式の移行先** | **Claude Mythos 5（`claude-mythos-5`）** ＝ access-gated の後継。GA 相当版は **Claude Fable 5** |
| **移行の容易さ** | モデル ID 変更でほぼドロップイン（同一 Messages API・同一トークナイザ）。ただし Mythos 5 は手動 extended thinking 非対応（adaptive thinking 常時 ON）・assistant prefill 不可・raw CoT 非返却など差異あり |

**ここに矛盾があります。** 公式の移行先である **Mythos 5 / Fable 5 は、2026年6月12日の米政府・輸出管理指令で全顧客向けに停止中**（前述の【2026-06-13追記】）。つまり **2026-06-21 時点では、初代 Mythos Preview の廃止日（6/30）が迫る一方で、推奨される移行先（Mythos 5）が利用できない**という異例の状況です。Anthropic 幹部は「数日以内の再有効化に自信」と述べていますが（→ 専用記事の続報）、**停止解除の具体的な期日も、廃止日（6/30）が延期されるか予定どおり実施されるかも、2026-06-21 時点では公表されていません**。

> **用語の整理**: 「**停止（suspension）**」（輸出管理による一時的な無効化／Mythos 5・Fable 5 が対象）と「**廃止（deprecation / retirement）**」（ライフサイクル上の計画的な提供終了／Mythos Preview が対象）は別概念です。Mythos Preview を利用中の場合は、停止解除の動向を注視しつつ、廃止日（6/30）前に Mythos 5 への移行準備（手動 thinking 設定の削除など）を進めておくのが安全です。

### 【2026-06-27追記】Mythos 5 が一部再開 — 米政府が100機関超への限定提供を許可

2026年6月26日（金）午後、米商務長官 Howard Lutnick が Anthropic に書簡を送り、**Mythos 5 を「信頼できるパートナー（certain trusted partners）」へ提供することを許可**しました。対象は **100以上の米国企業・政府機関**。Lutnick は「適切なセーフガードが整っており、信頼できるパートナーが Claude Mythos 5 モデルにアクセスすることを認める」と述べています。

一方、**コンシューマー向けの Fable 5 は書簡で言及されず、引き続き全面停止のまま**です（関係者によれば Fable も解除に向かっているが、時期は不明）。

これにより、上記の「初代 Mythos Preview の廃止日（6/30）が迫る一方で移行先（Mythos 5）が使えない」という矛盾は、**重要インフラ防衛用途への限定提供という形で部分的に解消に向かいつつあります**（一般・コンシューマー向けの全面解除ではない点に注意。Mythos Preview 自体の廃止日が変更されたかは未公表）。出典: [Semafor](https://www.semafor.com/article/06/27/2026/us-releases-powerful-anthropic-model-mythos-to-some-us-companies) / [CNBC](https://www.cnbc.com/2026/06/26/us-government-anthropic-claude-mythos5-ai.html)。

### 【2026-07-01追記】輸出管理が全面解除 — Fable 5 全世界再開・Mythos 5 限定復旧が確定

2026年6月30日（PT）、**米商務省が Fable 5・Mythos 5 の輸出管理制限を全面解除**しました。6/27 追記の「Mythos 5 の限定提供許可」から一気に進展し：

- **Fable 5 は 2026年7月1日、全世界で再開**（Claude.ai / Claude Platform / Claude Code / Claude Cowork。7/7 まで Pro/Max/Team・一部エンタープライズで週間上限の最大50%まで）。
- **Mythos 5 は米国の一部組織向けに限定復旧**（本記事の主題）。Anthropic は Glasswing プログラムを通じて国内外パートナーへのアクセス拡大を継続するとしています。
- 停止期間は約19日間（6/12 停止 → 7/1 再開）。トリガーは Amazon の研究者が Fable 5 で発見した jailbreak で、再開時に**新しいサイバーセキュリティ分類器**を追加したと報じられています。
