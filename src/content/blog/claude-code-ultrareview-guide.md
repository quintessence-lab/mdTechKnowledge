...
| コスト | 通常の利用枠を消費 | 1 回 $5〜$20（Extra usage） |
...
`/ultrareview` は**プラン込みの利用枠とは別**に、Extra usage（追加利用）として課金されます。**Pro / Max に提供されていた無料ラン（3 回）は 2026-05-05 をもって失効**し、現在は全プランで課金が発生します。

| プラン | 課金 |
|--------|------|
| Pro | Extra usage で課金 |
| Max | Extra usage で課金 |
| Team / Enterprise | Extra usage で課金 |

### 8.1 課金後の単価

変更規模に応じて **1 回あたり $5〜$20** の追加利用課金です。Team / Enterprise は Extra usage 機能を有効化していないとそもそも起動できません（`/extra-usage` で確認・有効化可能）。
...
- CI で全 PR に強制すると費用がかさむため、**ラベル `ultrareview` が付いた PR のみ**で発火させるのが現実的
...
## 10.5 Code Review（PR自動レビュー機能）との比較

2026年3月9日にローンチされた **Code Review** は、`/ultrareview` と同じくマルチエージェントによる深掘りレビューを行いますが、**GitHub App 統合による PR 自動監視型**という点で `/ultrareview` とは大きく性格が異なります。両者は競合ではなく、**手動ゲート（`/ultrareview`）と自動ゲート（Code Review）の二段構え**として使い分けるのが定石です。

### 比較表

| 観点 | `/ultrareview`（v2.1.111〜） | Code Review（2026-03-09〜） |
|------|------------------------------|------------------------------|
| 動作方式 | **手動呼び出し**（コマンド / CLI / CI） | **自動**（GitHub App が PR 作成・更新を監視） |
| 起動経路 | `/ultrareview` コマンド、`claude ultrareview` CLI、CI スクリプト | GitHub App をインストール → 対象リポジトリで自動発火 |
| 統合形態 | Claude Code セッション / CI ランナー | GitHub App（PR コメント・インラインコメントとして直接投稿） |
| 対象プラン | 全プラン（Pro / Max / Team / Enterprise） | **Team / Enterprise（リサーチプレビュー）** |
| 課金モデル | Extra usage、1 回 $5〜$20 | **トークン課金、平均 $15〜25 / レビュー**（PR 規模で変動） |
| 上限・制御 | 個人がコマンド単位で実行判断 | **月額上限・リポジトリ別制御**を組織管理者が設定可 |
| アーキテクチャ | クラウドサンドボックス上のレビューアフリート + 独立検証 | 複数エージェントが PR に対しチーム体制で並列バグハント + 重要度ランク付け |
| 出力先 | セッション内 findings / `bugs.json`（`--json`） | PR への単一コメント + インラインコメント |
| 主な向き先 | マージ前ゲート、CI 任意トリガー、深掘り検証 | **全 PR を自動監視**したい組織標準のレビュー基盤 |
| 既存軽量版との関係 | ローカル `/review` と二刀流 | 従来の GitHub Actions ベースのオープンソース版が軽量代替として継続 |

### どちらを選ぶか

- **個人 / 小規模チーム**: `/ultrareview` を「ここぞ」の局面で手動起動。コスト透明性が高く、レビュー対象を選別できる
- **Team / Enterprise で全 PR を統一品質ゲートにかけたい**: Code Review を GitHub App として常時起動し、月額上限とリポジトリ別制御でガバナンスを効かせる
...
- **コスト感**: 全プラン 1 回 $5〜$20（Extra usage）
