...
2026年5月6日（PT）、AnthropicはSpaceX傘下のxAIが運営するColossus 1データセンター（Memphis, TN）の全計算能力（300MW超、NVIDIA GPU 22万台超）を確保したと発表した。この大規模コンピュート拡張に伴い、**同日付でレート制限が大幅に引き上げられた**。

### 変更内容（2026年5月6日付）

| 変更項目 | 変更内容 |
| --- | --- |
| **Claude Code 5時間レート制限** | **Pro / Max / Team / Enterprise すべてで2倍に拡張** |
| **Claude Code ピーク時制限** | **Pro / Max で廃止** |
| **Claude Opus API レート制限** | 大幅引き上げ |

### Rate Limits API への影響

この変更はRate Limits APIを活用しているシステムにとって以下の意味を持つ。

- **自動追従のメリットが即座に実証**: 組織エンドポイント（`GET /v1/organizations/rate_limits`）で取得できる `requests_per_minute` 等の値が引き上げ後の新しい上限値に更新された。ハードコードされていたゲートウェイはこの恩恵を受けられないが、Rate Limits APIで動的取得していたシステムは自動的に新上限を反映できる。
- **ゲートウェイのリフレッシュ推奨**: 「ベストプラクティス」で推奨した起動時取得＋1時間ごとリフレッシュのパターンを実装していた場合、最大1時間以内に新しいレート制限値がシステムに反映される。
- **Opus 系 model_group の値が急上昇**: 特に `requests_per_minute`、`input_tokens_per_minute`、`output_tokens_per_minute` の値をモニタリングしている場合、急激な上昇を検知することがあるが、これは Anthropic 側の意図的な調整であり正常な変更である。
...
2026年5月13日（PT）／14日（JST）に発表され、**2026年6月15日（PT）施行**予定だった **Agent SDK credits** は、サブスクリプション枠の課金構造を「インタラクティブ利用」と「プログラム的利用」の2系統に分離する変更です。Rate Limits API の運用にも影響するため本記事でも収録します。

> ⚠️ **【重要・2026-06-27 追記】この課金分離は施行当日（2026-06-15）に一時停止されました。** Anthropic 公式は「変更を一時停止する。当面、何も変わらない。Agent SDK・`claude -p`・サードパーティアプリの利用は引き続きサブスクリプションの利用上限から消費される」と表明しています（[公式サポート](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan)）。したがって**以下の「分割内容」「Agent SDK credits」は当面は適用されていません**（恒久撤回ではなく一時停止で、将来再導入の可能性あり）。本節は経緯の記録として残します。
...
## 2026年6月26日 大改定 — Sonnet/Haiku が Opus と同水準・ティアを Start/Build/Scale の3段階へ統合

2026年6月26日（PT）／27日（JST）、Anthropic は Claude API のレート制限を大幅に引き上げました。本記事が扱う Rate Limits API の値そのものに直結する重要変更です。最大のポイントは2つです。

1. **Sonnet / Haiku のレート制限が、すべてのティアで Opus と同一水準**になりました。同一化されたのは **RPM（毎分リクエスト数）・ITPM（毎分入力トークン数）・OTPM（毎分出力トークン数）の3指標すべて**です。
2. これまでの利用ティアが **Start / Build / Scale の3段階に統合**されました（さらに上位に **Custom**）。

既存組織は **大半が自動的に上位ティアへ移行**し、**従来より低い制限になる組織はなく、操作も不要**とアナウンスされています（現在のティアは Claude Console > Settings > Limits で確認可）。

### 新ティアの主要レート制限（Opus 4.x / Sonnet 4.x / Haiku 4.5 共通）

| ティア | RPM | ITPM | OTPM | 月間上限額 |
|---|---|---|---|---|
| **Start** | 1,000 | 2,000,000 | 400,000 | $500 |
| **Build** | 5,000 | 5,000,000 | 1,000,000 | $1,000 |
| **Scale** | 10,000 | 10,000,000 | 2,000,000 | $200,000 |

※ Opus のレート枠は Opus 4.8/4.7/4.6/4.5 の合算、Sonnet は Sonnet 4.6/4.5 の合算に適用されます。Fable 5 は別枠（より低い上限）。Scale を超える要件は Custom ティア（営業窓口）で対応します。Claude Platform on AWS の組織は Start ティア固定です。

> **Rate Limits API への影響**: 本改定で各 `model_group` の返却値（RPM/ITPM/OTPM）が引き上がります。とくに **Sonnet/Haiku 主体の組織は実効スループットが大きく改善**するため、従来の充足率アラートのしきい値は見直しが必要です。なお旧「Tier 1〜4」と新「Start/Build/Scale」の1対1対応は公式に未掲載のため、対応関係は断定せず Console の実値で確認してください。

出典: [Anthropic Rate limits](https://platform.claude.com/docs/en/api/rate-limits) ／ [Release notes](https://platform.claude.com/docs/en/release-notes/overview)

## 2026年7月1日 引き上げ — Claude Sonnet 5 と higher effort levels に対応

2026年7月1日、**Claude Sonnet 5 のリリースと同時に、Chat / Cowork / Claude Code / Claude Platform の全体でレート制限が引き上げ**られました。Sonnet 5 は effort level を調整でき、**higher effort levels ほどトークン消費が増える**ため、その高負荷に見合うよう上限を引き上げた形です。

5月（SpaceX Colossus）・6月26日（Start/Build/Scale 統合）に続く上限拡大で、**Rate Limits API（`GET /v1/organizations/rate_limits`）で動的取得している組織は、この引き上げを自動的に反映**できます（ハードコードは恩恵を受けられない）。出典: [Introducing Claude Sonnet 5](https://www.anthropic.com/news/claude-sonnet-5) ／ [Release notes](https://platform.claude.com/docs/en/release-notes/overview)。

## Claude Code 週間利用上限 +50% プロモーション（〜2026年8月19日に再延長）

Anthropic は、**Claude Code の週間利用上限を50%引き上げるプロモーション**を実施しています。当初 7月19日までの予定でしたが、**2026年7月18日発表で8月19日まで再延長**されました。GPT-5.6 Sol など競合の価格圧力への対応と見られています。

| 項目 | 内容 |
|:---|:---|
| 対象 | **Claude Code のみ**（CLI・IDE拡張・Desktopアプリ・Web すべて）。Claude（web/desktop/mobile）や Cowork の上限は対象外 |
| 対象プラン | Pro / Max / Team・**legacy 席課金型 Enterprise**（Free・従量課金型 Enterprise 席は対象外） |
| 有効化操作 | **不要**。対象プランなら自動適用 |
| 期限 | **2026年8月19日**（当初7/19から再延長） |
| 期限後 | 週間上限は**追加操作なし・プラン変更なしで通常水準に戻る** |

本プロモーションは Rate Limits API が返す `requests_per_minute` 等の**上限値そのものには現れず**、週次の**利用量上限（usage quota）側の一時的な引き上げ**である点に注意してください。API 経由でヘビーに Claude Code を使っている組織は、**8/19以降の急な上限低下**を見込んでおくと安全です。出典: [DigitalApplied: Claude Code Limits +50%, Fable 5 Lands in Max and Team](https://www.digitalapplied.com/blog/claude-code-limits-raised-fable-5-max-team-premium-2026)。
...
- Anthropic の上限調整への**自動追従**（2026年5月の大幅向上もAPIで自動反映）
