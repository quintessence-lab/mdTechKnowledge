...
## Claude Code 週間利用上限 +50% プロモーション（〜2026年7月19日）

Anthropic は、**Claude Code の週間利用上限を50%引き上げるプロモーションを2026年7月19日 23:59（PT）まで延長**しています。
...
本プロモーションは Rate Limits API が返す `requests_per_minute` 等の**上限値そのものには現れず**、週次の**利用量上限（usage quota）側の一時的な引き上げ**である点に注意してください。API 経由でヘビーに Claude Code を使っている組織は、**7/19以降の急な上限低下**を見込んでおくと安全です。
