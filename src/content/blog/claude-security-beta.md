...
## 公開3週間時点の実績数値（2026-05-22 公表）

2026年5月22日（PT）/ 5月23日（JST）、パブリックベータ開始（2026-04-30）から **3週間** の運用実績が公表されました。同時に **全 Anthropic Enterprise カスタマーへの提供拡大** も確定しています。

| 項目 | 数値 |
|:-----|:-----|
| パブリックベータ開始 | 2026年4月30日（PT） |
| 公表時点の経過期間 | **約3週間** |
| **Opus 4.7 が適用した脆弱性パッチ累計** | **2,100件以上** |
| 提供範囲 | **Anthropic Enterprise 全カスタマー** に拡大（従来は先行招待のみ） |
| 主要報道 | Techzine, SiliconANGLE, Cyber Kendra, Help Net Security |

### 数値の意味

3週間で2,100件超 → **1週間あたり約700件のパッチ生成**ペース。これは「検出だけでなく **修正案の生成まで含めた件数**」であり、従来型 SAST との大きな差別化要素である「パッチ生成の自動化」が現場で実利用されていることを示唆します。

ベータ初期は「検出して終わり」ではないか、という懸念があったものの、本数値は **検出 → パッチ生成 → 採用** のループが回り始めていることを意味し、Phase 2〜3（限定運用〜全社展開）に進める組織側の信頼性が増したと評価できます。

### Enterprise 全カスタマー展開の含意

これまで「Claude Enterprise 契約を持つ顧客のうち、優先公開対象」に限られていましたが、本公表により **Enterprise 全カスタマーが即時利用可能** となりました。Claude Team / Max への展開時期は引き続き未定ですが、Enterprise 層では本格的な PoC / 本番運用フェーズに移行できる体制が整っています。
