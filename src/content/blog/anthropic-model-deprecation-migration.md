...
> ## ✅ 【実施済み】Sonnet 4 / Opus 4 はリタイア完了
>
> **2026年6月15日（PT基準の日付）に `claude-sonnet-4-20250514`（Sonnet 4）と `claude-opus-4-20250514`（Opus 4）がリタイアしました。** リタイア日を過ぎたため、これらのモデルIDを指定したAPIリクエストは**失敗（エラー応答）**します（[公式: Model deprecations](https://platform.claude.com/docs/en/about-claude/model-deprecations)）。**まだ移行が済んでいない場合は、本番・CI・バッチで旧モデルID（`claude-sonnet-4-0` / `claude-opus-4-0` / `-20250514` サフィックス）が残っていないか至急確認してください。**
>
...
|:---|:---|:---|:---|
| `claude-opus-4-8` | **Active（最新フラッグシップ）** | — | 2027-05-28 以降 |
| `claude-opus-4-7` | Active | — | 2027-04-16 以降 |
| `claude-opus-4-6` | Active | — | 2027-02-05 以降 |
| `claude-opus-4-5-20251101` | Active | — | 2026-11-24 以降 |
| `claude-opus-4-1-20250805` | **Retired（2026-08-06追記・完了確認）** | 2026-06-05 | **2026-08-05** |
| `claude-opus-4-20250514` | **Retired（完了）** | 2026-04-14 | **2026-06-15（完了）** |
...
>
> **【2026-08-06追記】予定通りリタイア完了を確認**: 公式の[Model deprecationsページ](https://platform.claude.com/docs/en/about-claude/model-deprecations)で、`claude-opus-4-1-20250805` のステータスが **`Retired`** になっていることを確認しました。**未移行のリクエストは現在エラーを返します。** 移行先は引き続き `claude-opus-4-8`（同額の `claude-opus-5` も選択可）です。
...
リタイア日（2026-06-15）は経過済みのため、**旧モデルへの戻しは原理的に不可**です（6/15 以前はロールバック可能でした）。**未移行の箇所はフォールバック（後述の戦略3）を入れたうえで即時に切替**てください。
...
## 10. 【2026-06-06追記】同日（6/15）に発生する**予定だった** Agent SDK / `claude -p` の課金分離（※施行当日に一時停止）

> ⚠️ **【2026-06-27 訂正】この課金分離は施行当日（2026-06-15）に一時停止されました。** Anthropic 公式は「変更を一時停止する。当面、何も変わらない。Agent SDK・`claude -p`・サードパーティアプリの利用は引き続きサブスクリプションの利用上限から消費される」と表明しています（[公式サポート](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan)）。したがって本節の「施行済み」記述は当面は適用されていません（恒久撤回ではなく一時停止で、将来再導入の可能性あり）。経緯の記録として残します。

モデル廃止（6/15）と**同じ日**に施行予定だった、サブスクリプション利用のもう一つの大きな変更です。**Agent SDK・`claude -p`（headless）などの「自動実行（エージェント）系」利用を、サブスクリプションの共有枠から切り離し、独立した「Agent SDK クレジットプール」（API 従量レート課金）へ移行**する内容でした（※上記のとおり一時停止中）。
...
>
> 注: 移行に「一回限りのオプトインが必要」とする解説もありますが、一次・二次ソース間で記述に差があります。自動適用される可能性が高いため、**6/15 を待たずに公式案内で自組織の扱いを確認**してください。

詳細な意思決定（自動実行の維持／対話運用への切替／API 直契約への移行）は、本記事とは別に専用ガイド記事での整理を予定しています。

---

## 11. 【2026-07-24追記・削除実施】Opus 4.7 fast mode を削除 — Opus 4.8 へ移行

「モデルの廃止」とは別に、**モデルの特定モード（fast mode）の廃止**も発生しています。2026年6月25日（PT）に非推奨化が予告されていた **Claude Opus 4.7 の fast mode（`speed: "fast"`）が、2026年7月24日に削除されました**。`claude-opus-4-7` に `speed: "fast"` を指定したリクエストは**エラーを返します**（Opus 4.7 自体は **standard 速度では引き続き利用可能**）。

移行は容易で、**モデルIDを `claude-opus-4-7` から `claude-opus-4-8` に変更するだけ**です。`speed: "fast"` とベータヘッダー `fast-mode-2026-02-01` はそのまま使えます。fast mode 料金も Opus 4.8 の方が安く、移行にメリットがあります。

| モデル | fast mode 料金（入力 / 出力, per MTok） | 状態 |
|---|---|---|
| **Opus 4.8** | $10 / $50 | 推奨（移行先） |
| Opus 4.7 | $30 / $150 | **2026-07-24 削除済み**（`speed:"fast"` は**エラー**） |
| Opus 4.6 | $30 / $150 | 2026-06-29 削除（以降は**standard 速度へフォールバック**＝エラーにはならない） |

> **挙動の違いに注意**: Opus 4.7 fast mode は削除後**エラー**、Opus 4.6 fast mode は削除後**standard 速度へフォールバック**します。4.7 を使っていた場合は速やかに Opus 4.8 へ移行してください。
>
> **【2026-06-29】GitHub Copilot でも Opus 4.8 fast mode が Preview 開始**（[GitHub Changelog](https://github.blog/changelog/2026-06-29-claude-opus-4-8-fast-mode-is-now-in-preview-for-github-copilot/)）。Copilot 経由で 4.7 fast 相当を使っていた場合の受け皿も 4.8 に揃いつつあります。

出典: [Fast mode — Claude Platform Docs](https://platform.claude.com/docs/en/build-with-claude/fast-mode) ／ [Release notes](https://platform.claude.com/docs/en/release-notes/overview)
...
- **2026-06-15**（PT基準の日付・公式は時刻指定なし）: Sonnet 4 / Opus 4 が**リタイア済み**（旧モデルID指定はリクエスト失敗化） → 公式推奨は **Sonnet 4 → `claude-sonnet-4-6` ／ Opus 4 → `claude-opus-4-8`**（段階移行なら Opus 4.7 経由も可）。**未移行なら移行完了の確認を**
...
- **6/15のリタイアは完了済み**で旧モデル戻しは不可。未移行ならフォールバックを挟んで即時に移行を完了させること
- **6/15予定だった課金分離は施行当日に一時停止**: Agent SDK・`claude -p`・GitHub Actions など自動実行系を独立クレジットプール（Pro $20／Max 5x $100／Max 20x $200）へ分離する予定だったが、**2026-06-15 に一時停止**され当面はサブスク枠のまま（恒久撤回ではない・将来再導入の可能性あり。§10）
- **Opus 4.7 fast mode は 2026-07-24 削除**: `speed:"fast"` を使う場合は **Opus 4.8 へ移行**（モデルID差し替えのみ・fast 料金も安い）。削除後 4.7 はエラー化（§11）
