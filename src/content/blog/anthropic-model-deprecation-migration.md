...
## 11. 【2026-06-25追記】Opus 4.7 fast mode の廃止予告（2026-07-24 削除）— Opus 4.8 へ移行

「モデルの廃止」とは別に、**モデルの特定モード（fast mode）の廃止**も発生しています。2026年6月25日（PT）、Anthropic は **Claude Opus 4.7 の fast mode（`speed: "fast"`）を非推奨化**し、**2026年7月24日に削除**すると予告しました。削除後、`claude-opus-4-7` に `speed: "fast"` を指定したリクエストは**エラーを返します**（Opus 4.7 自体は **standard 速度では引き続き利用可能**）。
...
| Opus 4.7 | $30 / $150 | **2026-07-24 削除**（以降 `speed:"fast"` は**エラー**） |
| Opus 4.6 | $30 / $150 | 2026-06-29 削除（以降は**standard 速度へフォールバック**＝エラーにはならない） |

> **挙動の違いに注意**: Opus 4.7 fast mode は削除後**エラー**、Opus 4.6 fast mode は削除後**standard 速度へフォールバック**します。4.7 を使っている場合は 7/24 までに必ず Opus 4.8 へ移行してください。
>
