...
モデル廃止（6/15）と**同じ日**に、Anthropic はサブスクリプション利用のもう一つの大きな変更を実施しました（**6/15 施行済み**）。**Agent SDK・`claude -p`（headless）などの「自動実行（エージェント）系」利用が、サブスクリプションの共有枠から切り離され、独立した「Agent SDK クレジットプール」（API 従量レート課金）へ移行**しました。モデルID移行の作業に追われている間に課金体系も変わるため、両方を同時に把握しておく必要があります。
...
- **6/15に課金体系も変更済み**: Agent SDK・`claude -p`・GitHub Actions など自動実行系が独立クレジットプール（Pro $20／Max 5x $100／Max 20x $200、ロールオーバーなし）へ分離（施行済み）。対話型 Claude Code・チャット・Cowork は対象外（§10）
