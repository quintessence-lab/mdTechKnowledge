...
## 【2026-08-14追記】Anthropic Risk Report: August 2026 — 未公開モデル「Model 2」と、ブレーキの物差しの限界

2026年8月14日（PT）、Anthropicは半期ごとのリスク自己評価レポート「**Anthropic Risk Report: August 2026**」を公開しました（2月版に続く2本目）。本記事のテーマである「売り物のブレーキは効くのか」に対して、**ブレーキそのものではなく“ブレーキを測る物差しと運用の限界”を自ら開示した**点で重要な文書です。

### ① 未公開の内部モデル「Model 2」の存在を開示

- Anthropic は Mythos 5 の後継として **Model 1・Model 2 の2つの内部モデル**を開発済みと開示。Model 2 は **Mythos 5 を上回る能力**を持ち（内部用途に関わるタスクで顕著な改善。研究者代替評価は 54.8%→62.8%）、社員に日常的に使われている
- 一方で **Model 2 の外部リリースは予定されていません**。アライメントリスクの増大が理由とされ、**「現行フロンティアより強力なモデルを公開しない」判断を Anthropic が下した初のケース**です
- 本シリーズの構図（Fable 5＝安全装置付きの一般公開版／Mythos 5＝承認組織限定）の**さらに上に、社外に出さない層が明示された**ことになります

### ② アライメントリスク評価を "very low" → "low" へ引き上げ

2月版レポートで "very low" だったアライメントリスク評価が、**"low" へ一段階引き上げ**られました。直近のサイバーセキュリティインシデント（本記事【2026-07-30追記】のサイバー評価インシデントを含む一連の事案）が根拠とされています。評論家の Zvi Mowshowitz は「自発的な開示自体は前向き」と評価しつつ、「実態は medium と見るべき」「生物兵器リスクの年率推定（0.2%）は低すぎる」と、自己評価の甘さを批判しています。

### ③ 内部安全ベンチマークの「飽和」

最も危険な能力閾値を検出するために設計された**内部安全ベンチマークが、モデルの能力向上に追いつけなくなった**（飽和した）ことを認めました。フォールバック安全設計（本記事冒頭）が機能しているかを検証する**物差し自体の更新が追いついていない**という構造的な課題です。

### ④ 生物化学分類器なしで約1年間処理されていたトラフィック

**2025年5月〜2026年4月の約1年間**、人間フィードバックベンダー向けのトラフィック（**約1.33億件のやり取り・約5万名の契約者**が関与）が、**生物化学分類器（biochem classifier）を通さずに処理されていた**ことが判明し、レポートで自己開示されました。第三者の指摘ではなく自発的開示である点は評価されている一方、「ブレーキの空白」が1年近く見過ごされていた事実は、本記事「ジェイルブレイク耐性 — 無敵ではない」の論点を運用面から裏付けるものです。
