---
title: "Claudeのマルチエージェント4系統 — DW・Agent View・Managed Agents・Agent Teams"
emoji: "🧭"
type: "tech"
...
> **🔗 関連記事との「系統数」の違い**：本記事は**マルチエージェントを動かす"機構"の4系統**（Dynamic Workflows / Agent View / Managed Agents / Agent Teams）に絞ります。**サブエージェント単体**を1系統として加えた**エージェント全般の5系統**の整理は [Agent Teams 大改訂 — Claude の『エージェント』5系統の全体マップ](https://note.com/mdtechknowledge/n/nb8f94e0bb1e1) を参照してください。観点が違うだけで内容は矛盾しません（本記事ではサブエージェントを「Dynamic Workflows が動かす実体」として扱うため独立系統に数えていません）。
...
本記事は **Dynamic Workflows を起点**に、Agent View・Managed Agents・Agent Teams との違いと関係を整理し、「どれをいつ使うか」の判断軸を示します。後半（補足）に、独立した解説記事を持たない **Agent View の機能詳細リファレンス**をまとめます（Agent Teams と Dynamic Workflows の1対1の深掘りは [専用記事](https://note.com/mdtechknowledge/n/nce2545c7c0a0) を参照）。
...
| 深掘り記事 | [DW 完全ガイド](https://note.com/mdtechknowledge/n/ndacb07ee23fa) | 本記事（後半 補足） | [Managed Agents ガイド](https://note.com/mdtechknowledge/n/n13473a66b658) | [Agent Teams vs DW](https://note.com/mdtechknowledge/n/nce2545c7c0a0) |
...
サブエージェントが「親に結果を返したら消える」のに対し、チームメイトは「対等な仲間として生き続け、対話を続ける」——これが両者を分ける核心です。詳細な1対1比較は [Agent Teams と Dynamic Workflows は何が違うのか](https://note.com/mdtechknowledge/n/nce2545c7c0a0) を参照してください。
...
「リーダーが実行役に定期的な状態報告を求め、チームメイトが報告する」といった**相互監視**が組めるのは、この双方向通信があるからです。有効化やチーム管理の手順は [Claude マルチエージェントの初期設定](https://note.com/mdtechknowledge/n/nc6a96fea435a) を参照してください。
