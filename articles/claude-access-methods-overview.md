...
emoji: "🚪"
type: "tech"
topics: ["claude", "claudecode", "ai", "anthropic", "初心者向け"]
published: true
canonical_url: "https://note.com/mdtechknowledge/n/nd1f327a486dc"
---
...
| **課金区分（2026/6/15〜）** | **インタラクティブ**（サブスク通常枠） | **インタラクティブ**（サブスク通常枠） | 対話利用は**インタラクティブ**／`claude -p`・SDK・GitHub Actionsは**プログラマティック**（独立クレジット） | GitHub連携の自動実行はおおむね**プログラマティック**寄り |

---

## 課金区分の分離（2026/6/15〜）

2026年6月15日より、Claudeサブスクリプションの利用が **「インタラクティブ利用」** と **「プログラマティック利用」** の2つの課金プールに分かれます。どの入り口を使うかだけでなく、**「人が対話的に使うのか」「プログラムから自動で叩くのか」** で課金先が変わる点に注意してください。
...
> **課金の注意（2026/6/15〜）**: 同じ端末の Claude Code でも、**対話的に使う分はサブスクの通常枠（インタラクティブ）**ですが、**`claude -p`（非対話実行）は別枠のプログラマティック・クレジット**から消費されます。詳しくは上記「課金区分の分離（2026/6/15〜）」を参照。
...
> **課金の注意（2026/6/15〜）**: GitHub Actions など**プログラムから自動で走らせるエージェント実行は、サブスク枠ではなく別枠のプログラマティック・クレジット**から消費されます。詳しくは上記「課金区分の分離（2026/6/15〜）」を参照。
...
- [MCPサーバーとClaudeの接続パターン解説](https://note.com/mdtechknowledge/n/nbedd5d6ce9df)
- [【重要Tips】Claude Codeの実行場所はどこ？](https://note.com/mdtechknowledge/n/n9e5924ae4945)
...
- [Anthropic's June 15 Billing Split — ChatForest](https://chatforest.com/reviews/anthropic-claude-agent-sdk-billing-split-june-15-2026-credit-pool-developer-guide/)
